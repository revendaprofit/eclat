...
- **Progresso por card em "Complete o conjunto"**: cada card de parceira (`complete-set/parceira.tsx`) guarda o próprio progresso; "Adicionar as duas" em duas parceiras diferentes adiciona a âncora duas vezes (linhas separadas, `conjunto_slot` de handles distintos) — comportamento esperado (dois conjuntos), não um bug de retry.
...
- **Resumo desdobrado** — `modules/common/components/cart-totals/index.tsx` (carrinho e checkout) e `modules/order/components/order-summary/index.tsx` (páginas do pedido) trocaram o bloco único "Desconto" por três blocos condicionais, cada um só quando > 0: **"Benefício Conjunto"**, **"Cupom"** e o **"Desconto"** residual, este último para a diferença entre `discount_subtotal` e a soma dos dois grupos (ruling 2 — ajustes que não são de linha, ex. frete); em `order-summary` o tipo `StoreOrder` não declara `discount_subtotal`, então a leitura cai para `order.discount_total` quando o campo não vier no objeto. Só `CartTotals` tem `data-testid` nessas linhas (`cart-beneficio-conjunto`, `cart-cupom`, `cart-discount`, todas com `data-value` **decimal**, mesma unidade de `cart-subtotal`/`cart-total`); `order-summary` não tem `data-testid`.
...
`apps/storefront/src/lib/util/carrinho-conjunto.ts` (sem I/O, sem React; 15 testes em `carrinho-conjunto.test.ts`): `PREFIXO_CONJUNTO`/`ehAjusteConjunto`, `LISTA_GATILHO`, `agruparDescontos` (devolve **centavos inteiros**, dividido por 100 só na exibição), `etiquetasDoCarrinho`, `etiquetaDoPedido`, `cuponsVisiveis`, `avisoCupom`, `montarGatilhos`, `tituloGatilho`, `slotGatilho` + tipos `AjusteLinha`, `LinhaComAjustes`, `GruposDesconto`, `UnidadeStore`, `ConjuntoFormadoStore`, `OportunidadeStore`, `Gatilho`. Storefront: **187 testes** (16 arquivos).

No Cockpit o espelho é `apps/cockpit/lib/pedido-conjunto.ts` (`agruparDescontosPedido`, `etiquetaConjunto`, `residualDesconto`, 5 testes; suíte do Cockpit em **41**). Diferença intencional e comentada nos dois módulos: `agruparDescontos` (storefront) devolve **centavos**, `agruparDescontosPedido`/`residualDesconto` (Cockpit) devolvem **decimal**, que é a unidade do `brl()` daquela tela.
...
2. **Resumo:** "Benefício Conjunto" = ajustes `CONJUNTO-*` das linhas; "Cupom" = os demais ajustes de linha; ajustes de método de envio não entram; a diferença cai na linha genérica "Desconto". Base do residual: `discount_subtotal` nos dois componentes do storefront (`CartTotals` e `order-summary`, este com *fallback* para `discount_total` quando o tipo/objeto não traz o campo); `discount_total` no Cockpit, por ser o único campo de desconto do pedido na Admin API que a tela já tinha. Cada linha só aparece quando > 0.
...
Rulings extras da fix wave F4 (2026-09-09): **Cockpit: "Itens" = `item_subtotal` + residual "Desconto"** — `item_subtotal` é o subtotal bruto (sem desconto/imposto) das linhas; `total` da linha já vem líquido dos ajustes, então usá-lo em "Itens" descontava Benefício Conjunto/Cupom duas vezes. **Sem slug de categoria em prosa** — `montarGatilhos` descarta a oportunidade inteira quando `nomesCategorias` não tem o handle da categoria faltante, em vez de cair para o handle (nunca mostrar um slug técnico no título do gatilho).
...
- **`oportunidades` no checkout sem timeout próprio** — `getCarrinhoConjunto(cart.id, countryCode, { comGatilhos: false })` chamado em `(checkout)/checkout/page.tsx` usa `cache: "no-store"` sem um timeout dedicado, mesma classe de exposição de `retrieveCart` (uma falha lenta do backend atrasa o render da página em vez de cair rápido no fallback vazio). Deferido.
- **Oportunidade sem nome de categoria resolvível é omitida** — `montarGatilhos` descarta a oportunidade inteira (não só o nome) quando `nomesCategorias` não tem o handle da `categoria_faltante`; o gatilho correspondente simplesmente não aparece no carrinho, em vez de mostrar o handle técnico em prosa.
...
## Vitrine com um card por cor (2026-09-14)

Decisão do dono: mais itens na vitrine sem duplicar cadastro. O produto continua **único** no Medusa (cor = opção de variante, estoque por SKU, conjuntos por produto, troca de cor na PDP); só a apresentação expande.

- **Listagem** (`/store`, categorias, coleções, busca e o destaque da home): `entradasPorCor` (`lib/util/listagem-cores.ts`, puro, testado) transforma os produtos já filtrados e ordenados em `ListingEntry { product, cor }`, uma por cor, cores do mesmo modelo lado a lado. Filtro de cor mantém só a cor escolhida; tamanho e "disponível" exigem variante disponível daquela cor; cor esgotada fica na grade sem filtros (card "Esgotado"). Produto de cor única ou sem opção Cor = uma entrada `cor: null`.
- `listProductsFiltered` devolve `entries` (não mais `products`): contagem "N peças", paginação (24) e `total` falam em cards; `computeFacets(..., porCor)` conta tamanho por cor disponível (um card por cor). Card: `ProductPreview cor` → `ProductCard initialColor` (abre na cor, link `?cor=`), nome da cor ativa sob o título quando o produto tem mais de uma cor. GA4: `entriesToItemList` com `item_variant` = cor; `select_item` também leva a cor.
- **Conjuntos por cor**: `montarCardsCuradoPorCor` (`lib/util/conjuntos.ts`) gera um card por cor em que TODAS as peças têm variante disponível (ordem/grafia da 1ª peça), com foto e preço daquela cor e sem a capa cadastrada (ela é de uma cor só); menos de duas cores em comum → card único de sempre. Link `hrefConjunto` = `/conjuntos/<handle>?cor=<cor>`; a página lê `searchParams.cor` e o builder pré-seleciona `corParceira(peça, cor)` em cada peça. Aparecem na página Conjuntos e **dentro da grade das listagens** (`getConjuntosDaListagem`: curados com ao menos uma peça no escopo), só sem filtros ativos, fora da busca e na última página, depois dos produtos; não entram na contagem de peças. O desconto continua valendo por produto, qualquer cor (regra do curado).
- **Interruptor** sem deploy: `site_content.listagem = { "cards_por_cor": boolean, "conjuntos_na_listagem": boolean }` (ausente = ligado, `listagemConfigDe`). `cards_por_cor: false` volta a um card por produto em todo lugar (inclusive conjuntos).
- Fora do escopo: "Mais {categoria}" na PDP e "Looks com essa peça" continuam por produto.
