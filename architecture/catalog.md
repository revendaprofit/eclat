...
1. **Adição sequencial** — loop `for` de `addToCart({ variantId, quantity: 1, metadata: slotMetadata(handle, i) })` na página do conjunto (`conjunto-builder.tsx`); dois passos `if` encadeados (peça 0, depois peça 1) em `parceira.tsx` (PDP) — nunca `Promise.all` (preserva a ordem dos slots).
2. **Progresso resumível, mecanismo diferente por fluxo:** na PDP ("Complete o conjunto", `parceira.tsx`, ruling V5) o progresso fica atrelado à seleção corrente, SEM `useEffect` de reset — uma chave derivada das variantes selecionadas (`` `${anchorVariantId}|${parceiraVariantId}` ``) + o conjunto de índices já adicionados; um retry com a MESMA seleção pula quem já entrou (sem duplicar linha); uma seleção NOVA começa do zero. Na página do conjunto (`conjunto-builder.tsx`) o mecanismo é outro: um `useEffect` zera `adicionados` quando `selecoes` muda, guardado por `adicionandoRef` para não disparar durante uma adição em voo (ruling V3, fix round 2) — mesma garantia de não duplicar linha num retry com a mesma seleção (a peça antiga adicionada na tentativa anterior fica no carrinho se a seleção mudar de propósito — a cliente ajusta manualmente se não quiser as duas, mesmo comportamento de trocar seleção na PDP comum).
