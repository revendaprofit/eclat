...
- Params de URL em português (spec §6.2), lidos/escritos por `apps/storefront/src/lib/util/catalog-filters.ts`: `tamanho, cor, preco, disponivel, ordenar, pagina`. Legado do starter (`sortBy`/`page`) é migrado por `mapLegacyParams` e devolve **redirect 308** para os novos params (sem quebrar links antigos).
