...
- Home — "Compre por peça" (`apps/storefront/src/modules/home/components/shop-by-category/index.tsx`, logo abaixo do `Hero` em `app/[countryCode]/(main)/page.tsx`): grade só de `nav.feminine` (grid 2 colunas no mobile, 5 no desktop), capa + nome, ordem = `rank`; sem produto feminino visível a seção retorna `null`. A reordenação por preferência de **estilo** do wizard "Minha ÉCLAT" (spec §10) é F5 — hoje a ordem é sempre por rank. A seção `Nossas linhas` (CMS, `home.lines` em `site_content`) continua disponível para campanhas e coexiste com "Compre por peça" — cabe ao dono decidir (Cockpit → Vitrine) se quer ocultá-la agora que a navegação por peça existe. Defaults de Treino/Casual removidos de `HOME_DEFAULTS`/`home/content.ts` e da vitrine fallback do Cockpit (`apps/cockpit/app/(painel)/vitrine/page.tsx`); `llms.txt` idem.
