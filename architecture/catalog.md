...
## PDP (Fase 3, 2026-09)
- Contexto de seleção (`apps/storefront/src/modules/products/components/product-selection`): `ProductSelectionProvider` monta com `key={product.id}` (remonta do zero a cada troca de produto), estado inicial só com `v_id` (SSR determinístico) e a preferência de tamanho do wizard entra depois de hidratar, sem sobrescrever o que a cliente já escolheu. Produto de variante única já vem pré-selecionado. Módulo puro `apps/storefront/src/lib/util/pdp-variants.ts` (`initialSelection`, `variantFor`, `selectedColor`, `sizeAvailability`, `isCompleteSelection`) concentra a lógica sem depender de DOM/React — testado em `pdp-variants.test.ts`.
- `?v_id`: só atualiza quando a seleção fica **completa** (cor + tamanho resolvem uma variante) — trocar só a cor não mexe na URL. Atualização via `window.history.replaceState` (nenhuma navegação de documento); `useSearchParams` não é ressincronizado por isso mas nada no client lê `v_id`, então é inofensivo.
- Seletores em `product-actions/color-select.tsx` (swatch com hex/nome) e `size-select.tsx` (P→GG fixo, independente da ordem no Medusa); tamanho sem estoque na cor escolhida aparece riscado/desabilitado com "Avise-me" ao lado.
- `NotifyMe` (`components/notify-me`) aparece quando a cor não tem nenhum tamanho disponível, quando o tamanho escolhido está esgotado naquela cor, ou ao clicar "Avise-me" na variante esgotada; grava em `avise_me` (Supabase, RLS anon INSERT-only) com `variant` = rótulo `"Cor / Tamanho"` (ex.: "Verde Exercito / GG"). `notifyFor` (o gatilho que decide mostrar o bloco) limpa a qualquer mudança de seleção — trocar de novo a cor ou o tamanho fecha o aviso anterior.
- Galeria: `variant-gallery` (client) aplica a regra `imagesForColor` (mesmo módulo usado no SSR de `page.tsx`, uma fonte só) para filtrar as fotos da variante selecionada; `persona-gallery` tem precedência quando a cliente escolheu uma persona no wizard "Minha ÉCLAT" (fotos geradas por IA substituem as fotos reais, com selo). `image-gallery` é quem desenha o carrossel com snap/indicadores no mobile; a lista de fotos muda → o carrossel reresseta para o primeiro slide.
- Medidas: `getCategoryPath` (`apps/storefront/src/lib/data/category-path.ts`) resolve a cadeia de categoria do produto a partir dos handles FLAT do Medusa (sobe por `parent_category_id` em memória, sem depender de rota aninhada) e alimenta `pickMeasurements(getMeasureMap())` — legging usa a chave `leggings` (Cintura/Quadril, sem Busto); subcategoria sem tabela própria herda da mãe.
- Breadcrumb: `apps/storefront/src/modules/common/components/breadcrumb` é a **única fonte** tanto do nav visível (Início › Legging › Legging Vertice) quanto do `BreadcrumbJsonLd` — mesmos itens, sem duplicidade de `BreadcrumbList` na página.
- Relacionados: "Mais {categoria}" — mesma categoria do produto, excluindo o produto atual, produtos da mesma cor primeiro, depois novidades, limite de 8.
- Eventos: `pushEcommerceEvent` (`apps/storefront/src/modules/analytics/push.ts`) é o helper único usado por `product-actions` (add to cart), quick-add do card e demais pontos de tracking — mantém o mapeamento herdado da Fase 2 (`item_variant` = "Cor / Tamanho").
- Critérios de aceite (spec §14, itens 6 e 10, escopo PDP) validados no navegador em 2026-09-08 — ver `progress.md`. Pendências fora do código: zoom/vídeo/thumbnails na galeria; parcelamento e CEP (aguardam Mercado Pago/Melhor Envio); avaliações reais (os depoimentos da PDP continuam fixos, sem nota agregada, decisão já registrada em 2026-07-28); "Complete o conjunto" (spec 2, ainda não escrita); fotos reais por cor (cada variante já tem imagem vinculada, mas hoje é o mesmo render para as 3 cores de um produto — troca de swatch não troca a foto visivelmente ainda); hex das cores em `site_content.cores` (Cockpit → Vitrine → Cores, ainda vazio); aviso do Next sobre `images.qualities` (o projeto usa `quality=80` nas imagens da PDP sem declarar `qualities` no `next.config.js` — cosmético, não bloqueia build).
