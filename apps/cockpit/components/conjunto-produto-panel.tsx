...
export default function ConjuntoProdutoPanel({
  ...
  if (!dados)
    return (
      <div>
        <label className={labelCls}>Conjuntos</label>
        <p className="text-meta text-eclat-texto-3">Carregando conjuntos…</p>
      </div>
    )
  ...
  return (
    <div className="border border-eclat-pedra/40 rounded-lg p-4 bg-white/60 flex flex-col gap-3">
      <h3 className="text-corpo font-medium text-eclat-texto">Conjuntos</h3>
      ...
      <div>
        ...
        {parceiras.length ? (
          <ul className="flex flex-col gap-2">
            {parceiras.map((p) => (
              <li key={p.id} className="flex items-center gap-2 text-corpo text-eclat-texto">
                {p.thumbnail ? (
                ...
                <span>{p.titulo}</span>
                <span className="text-meta text-eclat-texto-3">· {p.categoriaNome}</span>
              </li>
            ))}
          </ul>
        ) : (
          <p className="text-meta text-eclat-texto-3">Nenhuma peça parceira.</p>
        )}
      </div>
        ...
        {dados.curados.length ? (
          <ul className="flex flex-col gap-2 mb-1">
            {dados.curados.map((c) => {
              ...
              return (
                <li key={c.id} className="text-corpo text-eclat-texto flex flex-col">
                  <span>{c.nome}</span>
          ...
          <p className="text-meta text-eclat-texto-3 mb-1">Não faz parte de conjuntos curados.</p>
        )}
        <Link href="/conjuntos?aba=curados" className="text-meta text-eclat-dourado underline">
          ver conjuntos curados
        </Link>
      </div>

      {semNada && motivo && <p className="text-meta text-amber-700">{motivo}</p>}
    </div>
  )
}
