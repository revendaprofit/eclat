...
export default function FinanceiroPage() {
  ...
  const inputCls =
    "border border-eclat-pedra/50 rounded-md px-3 py-2 text-corpo bg-white focus:outline-none focus:border-eclat-dourado"

  return (
    <div>
      <div className="flex items-center justify-between mb-4 gap-4 flex-wrap">
        <h1 className="font-serif text-3xl text-eclat-texto">Financeiro</h1>
        <div className="flex items-center gap-2 text-corpo">
          <input type="date" value={de} onChange={(e) => setDe(e.target.value)} className={inputCls} />
          <span className="text-eclat-texto/40">até</span>
      ...
      {dre && (
        <div className="border border-eclat-grafite/20 rounded-lg bg-white/70 p-5 mb-6">
          <div className="flex items-baseline justify-between mb-3">
            <h2 className="font-serif text-xl text-eclat-texto">Resultado (DRE)</h2>
            <span className="text-meta text-eclat-texto-3">{dre.pedidos} pedido(s) · pagos/autorizados</span>
          </div>
          <div className="max-w-md flex flex-col gap-1.5 text-corpo">
            <div className="flex justify-between"><span className="text-eclat-texto-2">Receita de produtos</span><span>{brl(dre.receita_produtos)}</span></div>
            <div className="flex justify-between"><span className="text-eclat-texto-2">(−) CMV / COGS</span><span className="text-red-700">−{brl(dre.cogs)}</span></div>
            <div className="flex justify-between border-t border-eclat-pedra/30 pt-1.5 font-medium">
              <span>= Lucro bruto</span>
              <span>{brl(dre.lucro_bruto)} <span className="text-meta text-eclat-texto-3">({(dre.margem_bruta * 100).toFixed(1)}%)</span></span>
            </div>
            <div className="flex justify-between mt-1"><span className="text-eclat-texto-2">(+) Frete recebido</span><span>{brl(dre.frete)}</span></div>
            <div className="flex justify-between" data-testid="dre-taxas-pagamento"><span className="text-eclat-texto-2">(−) Taxas de pagamento</span><span className="text-red-700">−{brl(dre.taxas_pagamento ?? 0)}</span></div>
            <div className="flex justify-between"><span className="text-eclat-texto-2">(−) Despesas</span><span className="text-red-700">−{brl(dre.despesas)}</span></div>
          ...
          {dre.pagamentos_sem_tarifa > 0 && (
            <p className="text-meta text-amber-700 bg-amber-50 border border-amber-200 rounded-md p-2 mt-3" data-testid="aviso-sem-tarifa">
          ...
          {dre.itens_sem_custo > 0 && (
            <p className="text-meta text-amber-700 bg-amber-50 border border-amber-200 rounded-md p-2 mt-3">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
        <div className="border border-eclat-pedra/40 rounded-lg bg-white/60 p-4">
          <div className="text-meta uppercase tracking-wider text-eclat-texto-3">Despesas no período</div>
          <div className="font-serif text-3xl text-eclat-texto mt-1">{brl(total)}</div>
          <div className="text-meta text-eclat-texto-3 mt-1">{despesas.length} lançamento(s)</div>
        </div>
        <div className="md:col-span-2 border border-eclat-pedra/40 rounded-lg bg-white/60 p-4">
          <div className="text-meta uppercase tracking-wider text-eclat-texto-3 mb-2">Por categoria</div>
          {porCategoria.length === 0 ? (
            <p className="text-corpo text-eclat-texto-3">Sem despesas no período.</p>
          ) : (
            <div className="flex flex-col gap-1">
              {porCategoria.map(([nome, v]) => (
                <div key={nome} className="flex justify-between text-corpo">
                  <span className="text-eclat-texto-2">{nome}</span>
                  <span>{brl(v)}</span>
                </div>
              ))}
            </div>
      ...
      <div className="border border-eclat-dourado/40 rounded-lg bg-white/60 p-4 mb-6">
        <div className="flex items-center justify-between mb-3">
          <h2 className="font-serif text-xl text-eclat-texto">Lançar despesa</h2>
          <button onClick={() => setGerenciarCat((s) => !s)} className="text-meta text-eclat-dourado underline">
        ...
        {gerenciarCat && (
          <div className="mb-3 p-3 border border-eclat-pedra/30 rounded-md bg-eclat-areia/20 flex flex-wrap gap-2 items-center">
            {categorias.map((c) => (
              <span key={c.id} className="text-meta bg-white border border-eclat-pedra/30 rounded-full px-2 py-1 flex items-center gap-1">
                {c.nome}
                <button onClick={() => excluirCategoria(c.id)} className="text-eclat-texto/40 hover:text-red-700">✕</button>
              </span>
            ))}
            <button onClick={novaCategoria} className="text-meta text-eclat-dourado underline">+ categoria</button>
          </div>
        )}
        <div className="flex flex-wrap gap-2 items-end">
          <div>
            <label className="text-meta text-eclat-texto-3 block mb-1">Data</label>
            <input type="date" value={fData} onChange={(e) => setFData(e.target.value)} className={inputCls} />
          </div>
          <div>
            <label className="text-meta text-eclat-texto-3 block mb-1">Categoria</label>
          ...
          <div className="flex-1 min-w-[140px]">
            <label className="text-meta text-eclat-texto-3 block mb-1">Descrição</label>
            <input value={fDesc} onChange={(e) => setFDesc(e.target.value)} className={inputCls + " w-full"} />
          </div>
          <div>
            <label className="text-meta text-eclat-texto-3 block mb-1">Valor (R$)</label>
            <input value={fValor} onChange={(e) => setFValor(e.target.value)} inputMode="decimal" className={inputCls + " w-28"} />
          </div>
          <div>
            <label className="text-meta text-eclat-texto-3 block mb-1">Fornecedor</label>
            ...
            <tbody>
              {despesas.map((d) => (
                <tr key={d.id} className="border-b border-eclat-pedra/10 last:border-0 group">
                  <td className="px-4 py-2 text-eclat-texto-2">{new Date(d.data + "T00:00:00").toLocaleDateString("pt-BR")}</td>
                  <td className="px-4 py-2">{d.finance_expense_category?.nome ?? "—"}</td>
                  <td className="px-4 py-2">{d.descricao || "—"}</td>
                  <td className="px-4 py-2 text-eclat-texto-3">{d.fornecedor || "—"}</td>
                  <td className="px-4 py-2 text-right font-medium">{brl(d.valor_centavos)}</td>
                  <td className="px-4 py-2 text-right">
                    <button onClick={() => excluir(d.id)} className="text-meta text-red-700/70 hover:text-red-700 opacity-0 group-hover:opacity-100">excluir</button>
                  </td>
                </tr>
              ))}
              {despesas.length === 0 && (
                <tr><td colSpan={6} className="px-4 py-6 text-center text-eclat-texto-3 text-corpo">Nenhuma despesa no período.</td></tr>
              )}
            </tbody>
          </table>
        </div>
