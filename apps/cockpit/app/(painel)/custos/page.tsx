...
// Custos de Coleção — ficha de pré-custo por peça (prática de confecção):
// BOM (materiais/aviamentos) + facção + estampa + modelagem diluída + perdas
// => custo industrial; precificação com impostos/taxas/CAC (% do preço) e markup.
...
export default function CustosPage() {
  ...
  async function criar() {
    const name = prompt("Nome da peça (ex.: Calça Flare Licor):")
    if (!name) return
    const r = await fetch("/api/costing", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ name, collection: "Coleção 01" }),
    })
    if (r.ok) load()
  }
  ...
  return (
    <div className="flex flex-col gap-6">
      ...
      <div className="flex gap-6 items-start max-w-6xl">
        {/* lista */}
        <ul className="w-72 shrink-0 flex flex-col divide-y divide-eclat-pedra/30 border border-eclat-pedra/30 rounded-lg bg-white/60">
          {pieces.length === 0 && (
          ...
          {pieces.map((p) => {
            ...
            return (
              <li
                key={p.id}
                onClick={() => open(p)}
                className={`p-3 cursor-pointer hover:bg-eclat-areia/30 ${
                  sel?.id === p.id ? "bg-eclat-dourado/10" : ""
                }`}
              >
                <p className="text-sm text-eclat-grafite">{p.name}</p>
                <p className="text-xs text-eclat-grafite/50">
                  {p.collection || "—"} · custo {money(Math.round(pc.industrial))} ·{" "}
                  <span
                    className={
                      p.status === "aprovada"
                        ? "text-green-700"
          ...
          <div className="flex-1 border border-eclat-dourado/40 rounded-lg bg-white/70 p-5 flex flex-col gap-4">
            <div className="flex gap-3 items-center">
              ...
              <input
                ...
                placeholder="Coleção"
                onChange={(e) => setSel({ ...sel, collection: e.target.value })}
              />
              <select
                    ...
                    <tr key={i}>
                      <td className="py-1 pr-2">
                        <select
                          ...
                          onChange={(e) => {
                            const v = [...items]
                            v[i] = { ...it, kind: e.target.value as Item["kind"] }
                            setItems(v)
                          }}
                        >
                      ...
                      <td className="pr-2">
                        <select
                          ...
                          onChange={(e) => {
                            const v = [...items]
                            v[i] = { ...it, unit: e.target.value as Item["unit"] }
                            setItems(v)
                          }}
                        >
                      ...
                      <td className="pr-2">
                        <input
                          ...
                          onChange={(e) => {
                            const v = [...items]
                            v[i] = { ...it, consumption: parseFloat(e.target.value) || 0 }
                            setItems(v)
                          }}
                        />
                      </td>
                      <td className="pr-2">
                        <input
                          className={input}
                          defaultValue={(it.unit_price_centavos / 100).toFixed(2)}
                          onBlur={(e) => {
                            const v = [...items]
                            v[i] = { ...it, unit_price_centavos: parseMoney(e.target.value) }
                            setItems(v)
                          }}
                        />
                      </td>
            ...
            <div className="grid grid-cols-2 small:grid-cols-5 gap-3 text-xs text-eclat-grafite/60">
              <label>
                ...
                <input
                  className={input}
                  defaultValue={(sel.faccao_centavos / 100).toFixed(2)}
                  onBlur={(e) =>
                    setSel({ ...sel, faccao_centavos: parseMoney(e.target.value) })
                  }
                />
              </label>
              <label>
                ...
                <input
                  className={input}
                  defaultValue={(sel.estampa_centavos / 100).toFixed(2)}
                  onBlur={(e) =>
                    setSel({ ...sel, estampa_centavos: parseMoney(e.target.value) })
                  }
                />
              </label>
              <label>
                ...
                <input
                  className={input}
                  defaultValue={(sel.modelagem_total_centavos / 100).toFixed(2)}
                  onBlur={(e) =>
                    setSel({
                      ...sel,
                      modelagem_total_centavos: parseMoney(e.target.value),
                    })
                  }
                />
              </label>
              <label>
                ...
                <input
                  className={input}
                  type="number"
                  value={sel.modelagem_pecas}
                  onChange={(e) =>
                    setSel({
                      ...sel,
                      modelagem_pecas: Math.max(1, parseInt(e.target.value) || 1),
                    })
                  }
                />
              </label>
              <label>
                ...
                <input
                  ...
                  value={sel.perda_pct}
                  onChange={(e) =>
                    setSel({ ...sel, perda_pct: parseFloat(e.target.value) || 0 })
                  }
                />
              </label>
            </div>
            ...
            <div className="grid grid-cols-2 small:grid-cols-6 gap-3 text-xs text-eclat-grafite/60">
              ...
              <label>
                ...
                <input className={input}
                  defaultValue={(sel.frete_embalagem_centavos / 100).toFixed(2)}
            ...
            <label className="text-xs text-eclat-grafite/60">
              ...
              <input
                ...
                onChange={(e) =>
                  setSel({
                    ...sel,
                    medusa_variant_ids: e.target.value.split(",").map((s) => s.trim()).filter(Boolean),
                  })
                }
