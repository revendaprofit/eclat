...
const badge = (m: Record<string, { txt: string; cls: string }>, k: string) => {
  ...
  return <span className={`text-meta px-2 py-0.5 rounded-full ${b.cls}`}>{b.txt}</span>
}
  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-4 gap-4 flex-wrap">
        <h1 className="font-serif text-3xl text-eclat-texto">Clientes</h1>
        <span className="text-meta text-eclat-texto-3">{filtrados.length} cliente(s)</span>
      </div>
          ...
          return (
            <button
              ...
              className={`text-meta px-3 py-1.5 rounded-full border transition-colors ${
                seg === s.key
                  ? "bg-eclat-dourado/30 border-eclat-dourado text-eclat-texto"
                  : "border-eclat-pedra/40 text-eclat-texto-2 hover:bg-eclat-areia/40"
              }`}
            >
              {s.label} <span className="text-eclat-texto/40">{n}</span>
            </button>
          )
        })}
      </div>
      ...
      <input
        ...
        className="border border-eclat-pedra/50 rounded-md px-3 py-2 text-corpo bg-white focus:outline-none focus:border-eclat-dourado w-80 mb-4"
      />
              ...
              {filtrados.map((c) => (
                ...
                >
                  <td className="px-4 py-2">
                    ...
                    <div className="text-meta text-eclat-texto-3">{c.email}</div>
                  </td>
                  <td className="px-4 py-2 text-eclat-texto-2">{c.phone || "—"}</td>
                  <td className="px-4 py-2 text-center">{c.pedidos}</td>
                  <td className="px-4 py-2 text-right">{brl(c.total_gasto)}</td>
                  <td className="px-4 py-2 text-eclat-texto-3 text-meta">{data(c.created_at)}</td>
                </tr>
              ))}
              {filtrados.length === 0 && (
                <tr>
                  <td colSpan={5} className="px-4 py-6 text-center text-eclat-texto-3 text-corpo">
                    Nenhum cliente.
                  </td>
                </tr>
              )}
        ...
        <div className="fixed inset-0 z-50 flex justify-end bg-black/40" onClick={() => setFichaId(null)}>
          ...
          >
            <div className="sticky top-0 bg-eclat-luz border-b border-eclat-pedra/30 px-6 py-4 flex items-center justify-between z-10">
              <h2 className="font-serif text-2xl text-eclat-texto">Ficha do cliente</h2>
              <button onClick={() => setFichaId(null)} className="text-eclat-texto-3 hover:text-eclat-texto text-xl">✕</button>
            </div>
            ...
            {fichaLoading || !ficha ? (
              ...
              <div className="p-6 flex flex-col gap-6">
                {/* Identificação */}
                <section>
                  <h3 className="font-medium text-lg text-eclat-texto">{nomeDe(ficha.customer)}</h3>
                  <p className="text-corpo text-eclat-texto-3">{ficha.customer.email}</p>
                  <p className="text-corpo text-eclat-texto-3">{ficha.customer.phone || ficha.lead?.whatsapp || "sem telefone"}</p>
                  <div className="flex gap-4 mt-2 text-corpo">
                    <span><b>{ficha.orders.length}</b> pedido(s)</span>
                    <span>
                      total{" "}
                      <b>{brl(ficha.orders.reduce((s, o) => s + (o.total ?? 0), 0))}</b>
                    </span>
                    <span className="text-eclat-texto-3">desde {data(ficha.customer.created_at)}</span>
                  </div>
                </section>

                {/* Relacionamento */}
                {(ficha.lead || ficha.cliente_rel) && (
                  <section className="bg-white/60 border border-eclat-pedra/30 rounded-lg p-3 text-corpo flex flex-col gap-2">
                    <div className="flex items-center gap-2 flex-wrap">
                      {ficha.lead && (
                        <span className="text-meta px-2 py-0.5 rounded-full bg-eclat-dourado/20">
                          lead: {ficha.lead.status}
                        </span>
                      )}
                      {ficha.cliente_rel?.tags?.map((t) => (
                        <span key={t} className="text-meta px-2 py-0.5 rounded-full bg-eclat-areia/60">{t}</span>
                      ))}
                      {ficha.cliente_rel && (
                    ...
                    {ficha.cliente_rel?.notas && (
                      <p className="text-eclat-texto-2 text-meta">{ficha.cliente_rel.notas}</p>
                    )}
                    {ficha.lead?.conversation_id && (
                      <a
                        href={`/conversas?c=${ficha.lead.conversation_id}`}
                        className="text-meta text-eclat-dourado underline self-start"
                      >
                ...
                {ficha.addresses.length > 0 && (
                  <section>
                    <h4 className="text-meta uppercase tracking-wider text-eclat-texto-3 mb-2">Endereços</h4>
                    {ficha.addresses.map((a, i) => (
                      <p key={i} className="text-corpo text-eclat-texto-2">
                ...
                {/* Pedidos */}
                <section>
                  <h4 className="text-meta uppercase tracking-wider text-eclat-texto-3 mb-2">Pedidos</h4>
                  {ficha.orders.length === 0 ? (
                    <p className="text-corpo text-eclat-texto-3">Nenhum pedido.</p>
                  ) : (
                    <div className="flex flex-col gap-2">
                      {ficha.orders.map((o) => (
                        <div key={o.id} className="border border-eclat-pedra/30 rounded-md px-3 py-2 flex items-center justify-between text-corpo bg-white/50">
                          <div>
                            <span className="font-medium">#{o.display_id}</span>
                            <span className="text-eclat-texto-3 text-meta ml-2">{data(o.created_at)}</span>
                          </div>
                ...
                <section className="border border-eclat-dourado/40 rounded-lg p-4 bg-white/60 flex flex-col gap-2">
                  <h4 className="text-meta uppercase tracking-wider text-eclat-texto-3">Follow-up (WhatsApp)</h4>
                  <div className="flex flex-wrap gap-2">
                    {TEMPLATES_FUP.map((t) => (
                      <button
                        key={t.nome}
                        onClick={() => setFupText(t.texto(ficha.customer.first_name || "tudo bem"))}
                        className="text-meta border border-eclat-pedra/40 rounded-full px-3 py-1 hover:bg-eclat-areia/40"
                      >
                        {t.nome}
                      </button>
                    ))}
                  ...
                  <textarea
                    ...
                    className="border border-eclat-pedra/50 rounded-md px-3 py-2 text-corpo bg-white focus:outline-none focus:border-eclat-dourado resize-y"
                  />
                  <button
                    onClick={enviarFollowup}
                    disabled={fupSending || !fupText.trim()}
                    className="self-start bg-eclat-grafite text-eclat-luz uppercase tracking-widest text-meta px-5 py-2.5 rounded-md hover:bg-eclat-dourado hover:text-eclat-texto disabled:opacity-50"
                  >
