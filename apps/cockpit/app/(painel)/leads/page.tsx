  ...
  return (
    <div className="flex flex-col h-[calc(100vh-4rem)]">
      <div className="flex items-center justify-between mb-4">
        <div>
          <h1 className="font-serif text-3xl text-eclat-texto">Leads</h1>
          <p className="text-corpo text-eclat-texto-3">
          ...
          return (
            ...
            >
              <div className="px-3 py-2 border-b border-eclat-pedra/30 flex items-center justify-between">
                <span className="text-corpo font-medium">{stage.label}</span>
                <span className="text-meta text-eclat-texto-3">{items.length}</span>
              </div>
              <div className="flex-1 overflow-y-auto p-2 flex flex-col gap-2">
                {items.map((l) => (
                  ...
                  >
                    <div className="text-corpo font-medium truncate">{l.nome}</div>
                    <div className="text-meta text-eclat-texto-3">
                      {l.whatsapp || l.email || "—"}
                    </div>
                    <div className="mt-1 flex gap-1 flex-wrap">
                      {l.origem && (
                        <span className="text-meta uppercase tracking-wide bg-eclat-areia px-1.5 py-0.5 rounded">
                          {l.origem}
                        </span>
                      )}
                      {l.medusa_customer_id && (
                        <span className="text-meta uppercase tracking-wide bg-eclat-dourado/30 px-1.5 py-0.5 rounded">
                          cliente
                        </span>
                      )}
                    </div>
                ...
                {items.length === 0 && (
                  <p className="text-meta text-eclat-texto/40 px-1 py-2">—</p>
                )}
              </div>
            </div>
          )
        ...
        >
          ...
          >
            <div className="flex items-start justify-between">
              ...
              <button
                onClick={() => setSelected(null)}
                className="text-eclat-texto-3 hover:text-eclat-texto text-corpo"
              >
                Fechar
              </button>
            </div>
            ...
            <dl className="text-corpo flex flex-col gap-2">
              <div className="flex justify-between">
                <dt className="text-eclat-texto-3">WhatsApp</dt>
                <dd>{selected.whatsapp || "—"}</dd>
              </div>
              <div className="flex justify-between">
                <dt className="text-eclat-texto-3">E-mail</dt>
                <dd>{selected.email || "—"}</dd>
              </div>
              <div className="flex justify-between">
                <dt className="text-eclat-texto-3">Origem</dt>
                <dd>{selected.origem || "—"}</dd>
              </div>
              <div className="flex justify-between">
                <dt className="text-eclat-texto-3">Estágio</dt>
            ...
            <div className="flex flex-col gap-2">
              <button
                ...
                className="self-start border border-eclat-dourado/60 text-eclat-texto text-meta px-3 py-1.5 rounded-md hover:bg-eclat-dourado/15 transition-colors disabled:opacity-50"
              >
                {classifying ? "Analisando…" : "✨ Detectar estágio (IA)"}
              </button>
              {stageSuggestion && (
                <div className="border border-eclat-dourado/50 bg-eclat-dourado/10 rounded-md p-3 text-corpo flex flex-col gap-2">
                  <div>
                    Sugestão:{" "}
                    <strong>
                  ...
                  {stageSuggestion.motivo && (
                    <div className="text-meta text-eclat-texto-3">{stageSuggestion.motivo}</div>
                  )}
                  {stageSuggestion.estagio === selected.status ? (
                    <div className="text-meta text-eclat-texto-3">Já está neste estágio.</div>
                  ) : (
                    <div className="flex gap-2">
                      <button
                        onClick={aplicarEstagio}
                        className="bg-eclat-grafite text-eclat-luz text-meta uppercase tracking-widest px-3 py-1.5 rounded-md hover:bg-eclat-dourado hover:text-eclat-texto transition-colors"
                      >
                        Mover para{" "}
                        {STAGES.find((s) => s.key === stageSuggestion.estagio)?.label}
                      </button>
                      <button
                        ...
                        className="text-meta text-eclat-texto-3 px-2"
                      >
                        Ignorar
                      </button>
                    </div>
            ...
            <label className="flex flex-col gap-1 text-corpo">
              <span className="text-eclat-texto-3">Notas</span>
              <textarea
                ...
                rows={4}
                className="border border-eclat-pedra/50 rounded-md px-3 py-2 bg-white text-corpo focus:outline-none focus:border-eclat-dourado resize-none"
              />
              <button
                onClick={salvarNotas}
                disabled={busy}
                className="self-start text-meta uppercase tracking-widest text-eclat-texto hover:text-eclat-dourado disabled:opacity-50"
              >
                Salvar notas
              </button>
            </label>
            ...
            <div className="flex flex-col gap-2 mt-2">
              {selected.conversation?.[0]?.id && (
                <Link
                  ...
                  className="text-center border border-eclat-pedra/50 rounded-md py-2 text-corpo hover:bg-eclat-areia/40 transition-colors"
                >
                  Abrir conversa
                </Link>
              )}
              ...
              {selected.medusa_customer_id ? (
                <div className="text-center text-meta text-eclat-texto-3 border border-eclat-dourado/40 rounded-md py-2">
                ...
                <button
                  onClick={converter}
                  disabled={busy}
                  className="bg-eclat-grafite text-eclat-luz uppercase tracking-widest text-meta py-2.5 rounded-md hover:bg-eclat-dourado hover:text-eclat-texto transition-colors disabled:opacity-50"
                >
          ...
          >
            ...
            <div className="flex gap-2 justify-end mt-2">
              <button
                type="button"
                onClick={() => setShowNew(false)}
                className="text-corpo text-eclat-texto-3 px-3"
              >
                Cancelar
              </button>
              <button
                type="submit"
                disabled={busy}
                className="bg-eclat-grafite text-eclat-luz uppercase tracking-widest text-meta px-4 py-2 rounded-md hover:bg-eclat-dourado hover:text-eclat-texto transition-colors disabled:opacity-50"
              >
                Criar
              </button>
            </div>
