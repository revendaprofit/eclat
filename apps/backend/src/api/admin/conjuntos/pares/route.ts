...
import { normalizarPar } from "../../../../modules/beneficio-conjunto/utils/pares"
...
//
// Normalização (guarda `a !== b` + ordena `a < b`) fix round 1, achado "PUT /pares duplica a
// normalização de criarPar": não reimplementa a regra aqui — usa `normalizarPar`, a mesma função
// pura que `service.criarPar` usa, então as duas únicas portas de escrita de um `ConjuntoPar`
// (criação avulsa via serviço, e esta rota) nunca podem divergir sobre o que conta como "o mesmo
// par".
export const PUT = async (req: AuthenticatedMedusaRequest<AtualizarParesBody>, res: MedusaResponse) => {
  ...
  for (const p of body.pares) {
    const { categoria_a, categoria_b } = normalizarPar(p.categoria_a, p.categoria_b)
