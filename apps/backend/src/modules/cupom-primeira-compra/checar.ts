// Parte com I/O da regra "um cupom de primeira compra por CPF" (regra.ts tem a parte pura).
...
import { avaliarCupomPrimeiraCompra, cuponsDePrimeiraCompra, mensagemCupomJaUsado, normalizarCpf } from "./regra"
...
export async function recusaPorCupomDePrimeiraCompra(container: MedusaContainer, cartId: string): Promise<string | null> {
  ...
  // query.graph não faz. Só ids; os cupons vêm pelo vínculo pedido↔promoção.
  ...
  const linhas = (await pg("order")
    ...
    .whereNull("deleted_at")) as { id: string }[]
  if (!linhas.length) return null

  const { data: pedidos } = await query.graph({
    entity: "order",
    fields: ["id", "promotions.code"],
    filters: { id: linhas.map((l) => l.id) },
  })
  const r = avaliarCupomPrimeiraCompra(
    cupons,
    (pedidos as { promotions?: { code?: string | null }[] }[]).map((p) => ({ cupons: (p.promotions ?? []).map((x) => x?.code) }))
  )
  return r.permitido ? null : mensagemCupomJaUsado(r.codigo)
}
