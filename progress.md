...
## 2026-09-09 — F1 Benefício Conjunto: código concluído; deploy e seed pendentes de autorização
- Spec: `docs/superpowers/specs/2026-09-08-beneficio-conjunto-design.md` (§4, §5, §6 — amendada nesta task nos §4.4/§6.1/§6.4/§6.5/§6.6/§10/§12) · Plano: `docs/superpowers/plans/2026-09-08-conjunto-f1-backend.md` · Pasta de execução: `.superpowers/sdd/2026-09-08-conjunto-f1-backend/` (briefs + relatórios das 8 tasks) · SOP: `architecture/conjunto.md` (novo) · Branch `feat/conjunto-f1-backend`, HEAD antes desta task `76b1438`.
- Entrou (Tasks 1–7, sem tocar produção): módulo `beneficio-conjunto` (`apps/backend/src/modules/beneficio-conjunto`) com 3 modelos DML (`conjunto_regra`/`conjunto_par`/`conjunto_curado`, constraints de unicidade e CHECK no próprio banco) + `BeneficioConjuntoService` (`MedusaService` + `carregarAtivos`/`criarPar`); função pura `montarConjuntos` (curados primeiro, depois pares de coleção testando todas as ordens de processamento ≤ 6 pares e escolhendo mais conjuntos → maior desconto → ordem lexicográfica) e `descontoDoConjunto`/`regraEfetiva`; gancho real `setPromotionContext` (`conjunto-marcar.ts`, marca `conjunto_desconto` = `regra_id`\|`"conjunto"`\|`"nenhum"`, sem `conjunto_id`) substituindo o `conjunto-poc.ts` da F0 (removido); promoção automática por regra (`sincronizarPromocao`/`sincronizarTodas`) e conversão de cupom (`converterCupom`/`converterTodosCupons`) via os ganchos `promotionsCreated`/`promotionsUpdated` (não há evento de domínio no módulo Promotion 2.15.5); 9 rotas admin (`/admin/conjuntos/{regras,regras/[id],pares,curados,curados/[id],por-produto/[product_id],reconciliar}`) e 4 rotas store (`/store/conjuntos{,/[handle],/por-produto/[product_id],/oportunidades}`) com validação zod estrita; 30 testes unitários + 64 de integração HTTP, todos verdes contra o Postgres de teste em Docker (`eclat-pg-test`).
- Entrou (Task 8, esta): `scripts/setup-conjunto.py` (mesmo padrão de `setup-categorias.py`: regra padrão inativa se faltar, pares `leggings+tops`/`shorts+tops` preservando os demais, `POST /admin/conjuntos/reconciliar`, listagem best-effort de cupons já convertidos; validado com `python -m py_compile`, **não executado** contra produção); `architecture/conjunto.md` (SOP completo: dados, contrato do motor herdado da F0, fluxo do carrinho, cupons, rotas com exemplos JSON, testes Docker/Windows, `db:generate beneficioConjunto` em `eclat_dev`, riscos, consumo por F2/F3/F4, checklist de deploy); spec amendada nos rulings do controller que corrigiam o texto original (§4.4 ruling 2 — validação de curado só existência+published; §6.1 ruling 4 — sem `conjunto_id` no contexto; §6.4 ruling 3 — ganchos de workflow, não assinante de evento; §6.5 ruling 1 — rotas Store devolvem estrutura+ids, não produto com preço; §6.6 — sem `DELETE` em regras `padrao`/`colecao`); §10 marca F1 "implementada, aguardando deploy".
- PENDÊNCIAS (autorização do dono, nada executado nesta task): (1) `git push origin feat/conjunto-f1-backend` (política do projeto — agente entrega os comandos, dono faz push); (2) merge/deploy no Railway, que roda `medusa db:migrate` no `predeploy` e cria as 3 tabelas; (3) com "pode aplicar": `python scripts/setup-conjunto.py --apply` (regra padrão inativa + pares + reconciliação de cupons existentes); (4) validar `GET /store/conjuntos` em produção (vazio até existir regra ativa) e um carrinho de teste; (5) o dono ativa a regra padrão (Cockpit F2, quando existir, ou `PUT /admin/conjuntos/regras/:id { "ativa": true }`) quando quiser ligar o benefício. Checklist completo em `architecture/conjunto.md` §14. Próxima ação: deploy (F1) autorizado pelo dono, depois F2 (Cockpit) e F3 (vitrine), que podem correr em paralelo.

## 2026-09-09 — Benefício Conjunto F1: DEPLOY em produção ✅
- Backend NÃO está ligado ao GitHub no Railway (projeto `loyal-abundance`, serviço `endearing-enthusiasm`): o push de `main` não gera deploy. Deploy feito via CLI (`railway link -p loyal-abundance -e production` + `railway service endearing-enthusiasm` + `railway up --detach`), deployment `c50c49b2` SUCCESS 06:40. Antes disso o último deploy com sucesso era de 15/06 (um falho em 07/07): produção rodava o backend de junho.
- `railway up` falhou com 413 (214 MB): criado `.railwayignore` (sobe só o backend). `railway.json` da raiz com `healthcheckTimeout` 1200 (igual ao do backend; a migração roda no `predeploy`).
- Seed com "pode aplicar" (`python scripts/setup-conjunto.py --apply`): regra padrão `creg_01M22S8VMAMWT96ZE8GRQ6GJBY` (total_percentual 10, **inativa**) com promoção `CONJUNTO-creg_…` (inactive, automática, items); pares `leggings+tops` e `shorts+tops`; nenhum cupom existia → conversão não tocou nada. Verificado: `/store/conjuntos` → `{curados:[],colecoes:[]}` (regra inativa), `/store/products` 200.
- Pendências: o dono ativa a regra padrão quando quiser ligar o benefício (Cockpit F2 ou `PUT /admin/conjuntos/regras/:id {ativa:true}`); religar o GitHub ao serviço no Railway (Settings → Source) para deploys automáticos; cosmético: passo 4 do script conta a promoção CONJUNTO no denominador ("0 de 1").

## 2026-09-09 — Benefício Conjunto F2 (Cockpit): código concluído; validação visual pendente do dono
- Spec: `docs/superpowers/specs/2026-09-08-beneficio-conjunto-design.md` (§8) · SOP: `architecture/conjunto.md` §13 (novo) e `architecture/cockpit.md` §7 (novo) · Pasta de execução: `.superpowers/sdd/2026-09-09-conjunto-f2-cockpit/` · Branch `feat/conjunto-f2-cockpit`, HEAD antes desta task `9bf732f`.
- Entrou (Tasks 1–5, sem tocar produção): módulo puro `apps/cockpit/lib/conjunto.ts` (tipos, `TIPOS_DESCONTO`, conversão R$/% ↔ centavos, `validarRegra`/`validarCurado`, `previaBeneficio`, `alertaEstoque`, `slugConjunto`, `motivoSemConjunto`, `raizDeCategoria`) com 17 testes (`conjunto.test.ts`; suíte do cockpit sobe de 19 para **36**); helpers `conjunto*`/`MedusaHttpError` em `lib/medusa.ts` e `lib/api-erro.ts`; rotas `app/api/conjuntos/**` — proxies finos, status/mensagem do backend preservados (`400` de validação, `422 duplicate_error` para duplicidade); item de menu "Conjuntos (benefício)" e tela `/conjuntos` (abas `?aba=regras|curados`): `components/conjunto-regras.tsx` (benefício padrão com prévia ao vivo — exemplo fixo Top R$ 189 + Legging R$ 259 —, exceções por coleção só com o toggle "Ativa", pares permitidos em chips, "Reconciliar" com confirmação inline; trocar o tipo de desconto sempre limpa o campo de valor); `components/conjunto-curados.tsx` (lista ordenável por arrastar nativo persistindo `PUT { ordem }` por item movido, drawer com busca de produtos 300 ms + `AbortController`, badges de estoque via `alertaEstoque`, upload de capa via `/api/site-upload` usando `components/upload-imagem.tsx` extraído, prévia do card, `capa_url: null` para limpar, ids não resolvidos preservados ao editar com aviso); `components/conjunto-produto-panel.tsx` montado em `product-form.tsx` (modo edição, após "Fotos por cor") — motivos via `motivoSemConjunto`.
- Backend tocado nesta branch (Task 5): `apps/backend/src/api/middlewares.ts` — `capa_url` agora `.nullable()` nos schemas de criar/editar curado, para o Cockpit poder enviar `capa_url: null` (limpar capa) sem `400`. +2 testes de integração — suíte HTTP do backend em **70** no total (68 nos `conjunto-*.spec.ts` + 2 em `saude.spec.ts`).
- Rulings (plano, verbatim): proxies finos; produtos vêm de `GET /api/products?q=` com badges de `alertaEstoque` — estoque nunca bloqueia; ordem do drag persistida por item movido; prévia com exemplo fixo; sem tela de cupom, só o botão "Reconciliar". Rulings de execução: **C1/C3** — `capa_url` null = sem capa, `""` é rejeitado; **C2** — trocar o tipo de desconto limpa o valor; **C4** — ids não resolvidos (fora do teto de 100 do `GET /api/products`) preservados ao editar; **C5** — o motivo no painel da ficha vem da coleção/categorias do próprio produto, nunca da lista limitada a 100.
- Limites conhecidos: `GET /api/products` tem teto de 100 (curado em edição mostra aviso e preserva ids; título cai para o id quando não resolvido); não existe "Usar padrão" (remover exceção) porque o backend não tem `DELETE /admin/conjuntos/regras/:id` — pendência de backend registrada na spec e em `architecture/conjunto.md`; não há teste de componente React no projeto (validação = `tsc` + testes puros + roteiro do dono); `npm run lint` no cockpit está quebrado por um problema de toolchain preexistente (`ajv`/`eslintrc`), sem relação com esta fase.
- **Antes do roteiro: o backend precisa do redeploy (`railway up`) para aceitar curado sem capa.** Sem isso, `capa_url: null` continua devolvendo `400` em produção mesmo com o Cockpit já pronto para enviá-lo — ver comandos em `architecture/eclat-git-push.md`/nota de memória do projeto (o dono faz `git push`; o agente entrega os comandos do redeploy).
- **Roteiro de validação do dono** (Cockpit em produção, login do dono — nenhum destes passos foi executado nesta task):
  1. Abrir o Cockpit → Conjuntos.
  2. Aba Regras: conferir o benefício padrão vindo de produção (10% sobre o total, **inativo** — seed de `scripts/setup-conjunto.py --apply`, entrada anterior); editar para "% na peça de menor valor" 20 e **salvar sem ativar**; conferir a prévia (economia R$ 37,80 — 20% do Top R$ 189, a peça mais barata do exemplo).
  3. Conferir os pares Top + Legging e Top + Short listados; adicionar e depois remover um par de teste.
  4. Aba Conjuntos curados: criar "Look teste" com 2 produtos publicados, capa, tipo "R$ sobre o total do conjunto" 40,00; ver na lista; arrastar para reordenar; excluir.
  5. Abrir a ficha de um produto Top: o painel "Conjuntos" mostra as parceiras.
...
## 2026-09-09 — Benefício Conjunto F3 (Vitrine): código concluído + aceite local
- Spec: `docs/superpowers/specs/2026-09-08-beneficio-conjunto-design.md` (§7.1–7.4, 7.6, 7.7; §6.3 "Confirmado F3"; §11 itens 5–8) · SOP: `architecture/catalog.md` ("Conjuntos na vitrine — Fase F3, 2026-09") e `architecture/conjunto.md` §13 · Pasta de execução: `.superpowers/sdd/2026-09-09-conjunto-f3-vitrine/` (briefs + relatórios das 6 tasks) · Branch `feat/conjunto-f3-vitrine`.
- Entrou (Tasks 1–5): `lib/util/conjuntos.ts` puro (cards, `descontoConjunto` espelho do backend, `precoMinDisponivel`, `corParceira`, `elegibilidade`, `totalDoConjunto`, `slotMetadata`, `formatarReais`) + `lib/data/conjuntos.ts` (`getVitrineConjuntos`/`getConjunto`/`getConjuntosDoProduto`/`getElegibilidade`, hidratação em lote via `listProductsByIds`) + `addToCart` com `metadata`; página `/categories/conjuntos` (curados + pares gerados por coleção, grade de 12 + "ver todos"); página `/conjuntos/[handle]` (seleção por peça, rodapé com total/benefício, "Adicionar o conjunto" resumível); bloco "Complete o conjunto" + "Looks com essa peça" na PDP com "Adicionar as duas"; selo "Forma conjunto" no card (`product-listing.tsx` chama `getElegibilidade()` uma vez por listagem); `ConjuntoJsonLd`; `apps/backend/src/scripts/seed-dev-conjunto.ts` (seed local-only, guarda `DATABASE_URL`).
- Ajuste de backend nesta fase (ruling V2, Task 4): `apps/backend/src/modules/beneficio-conjunto/catalogo-conjuntos.ts` — `listarConjuntos`/`conjuntoPorHandle` omitem um par gerado por coleção quando seu conjunto de ids bate exatamente com o de um curado ativo (o carrinho aplicaria a regra do curado, não a da coleção) — consistência vitrine × carrinho. +2 testes de integração.
- Rulings do controller registrados na spec (verbatim, ver §7/§12 amendados): **V1** "a partir de" no card + `view_item_list` só dos cards inicialmente visíveis + fallback de thumbnail; **V2** par ofuscado por curado (acima); **V3/V5** "Adicionar o conjunto" (página do conjunto) / "Adicionar as duas" (PDP) resumíveis após falha parcial, por mecanismos diferentes — V5 (PDP): progresso atrelado à seleção corrente, sem `useEffect` de reset; V3 (página do conjunto): `useEffect` de reset guardado por `adicionandoRef` durante a adição; em ambos, erro nomeia a peça e seletores ficam travados durante a adição; **V4** selo "Forma conjunto" também para peça elegível só via curado ativo (fecha a ofuscação que o V2 introduzia na elegibilidade); **V6** (fix wave final) a mesma lógica do V2 aplicada à rota `por-produto` — a parceira coberta por um curado ativo com exatamente `{âncora, parceira}` sai de "Complete o conjunto" e fica só em "Looks com essa peça" (a PDP prometia um benefício que o carrinho nunca cobra); **V7** (fix wave final) o retry resumível virou UM mecanismo — módulo puro `apps/storefront/src/lib/util/adicao-conjunto.ts` (`pendentes`/`adicionarEmSequencia`/`mensagemFalha`, 11 testes) usado pelos dois fluxos, com progresso **por peça** (`{ [indice]: variantId }`): retry com a mesma variante pula quem já entrou, trocar a variante de uma peça re-adiciona só ela; substitui a chave composta do V5 e o `useEffect` de reset do V3 (a trava dos seletores, também V3, continua).
- **Confirmado F3 (spec §6.3):** `metadata.conjunto_slot` **não funde linhas** no Medusa 2.15.5 — repetir a adição do mesmo conjunto gerou 2 → 4 linhas (nunca 2 com quantidade 2), cada uma com `conjunto_slot` distinto. Fecha o limite conhecido da F1 (linha dividida compartilha a base de desconto com um cupom) **só para os fluxos da F3** (página do conjunto, "Complete o conjunto"); uma peça adicionada pelo botão comum "Adicionar à sacola" da PDP continua sem esse metadata e permanece sujeita ao limite antigo.
- **Suítes (após a fix wave final V6/V7):** storefront `npm test --workspace=apps/storefront` → **172 passed** (15 arquivos; era 161/14 antes do módulo `adicao-conjunto`). Cockpit `npm test --workspace=apps/cockpit` → **36 passed** (3 arquivos). Backend (de `apps/backend`, `npm_config_script_shell="C:\Program Files\Git\usr\bin\bash.exe"`): `npm run test:integration:http` → **73 passed** (8 suítes, container `eclat-pg-test`; era 72 antes do teste do V6); `npm run test:unit` → **30 passed** (3 suítes). `npx tsc -p apps/storefront --noEmit`, `npx tsc -p apps/cockpit --noEmit` e `npx tsc -p apps/backend --noEmit 2>&1 | grep "error TS" | grep -v cockpit/page.tsx` → todos limpos.
- **Aceite spec §11 itens 5–8** (navegador, backend local semeado — `eclat_dev`/`eclat-pg-test`, regra padrão 20% **ativa** só neste seed, ≠ produção):
  - **5 — PASSOU.** `/br/categories/conjuntos` lista o curado "Look Blackout" e o par gerado "Short Eclipse Blackout + Top Aura Blackout" (o par `legging×top` some por V2, esperado). Desativando o produto Short via Admin API local (`status: draft`) e recarregando, o par `short--top` some da vitrine (e "SHORTS" some do menu); restaurado (`status: published`) o par volta.
  - **6 — PASSOU.** `/br/conjuntos/look-blackout` (o par `legging×top` equivalente dá 404 por V2, como esperado — usado o curado + o par `short--top` para o restante do roteiro, conforme a nota do brief): escolhido tamanho M nas duas peças, "Adicionar o conjunto" → 2 linhas novas no carrinho com `conjunto_slot` distintos (`look-blackout#0#…`/`look-blackout#1#…`) e `adjustments` `CONJUNTO-creg_01M23BKRY1NC8S98JZ5G9ZP97J` de R$ 22,50 em cada uma (R$ 45,00 total, regra do curado) — confirmado via `GET /store/carts/:id?fields=*items,*items.adjustments` e leitura direta do Postgres.
  - **7 — PASSOU** (validado ANTES do ruling V6; ver nota abaixo). PDP `/br/products/top-aura-blackout` mostra "Complete o conjunto" (Short e Legging como parceiras) e "Looks com essa peça" (curado Look Blackout). **Nota V6 (fix wave final):** com o V6 no ar, a Legging Vértice Blackout deixa de aparecer em "Complete o conjunto" na PDP do Top (o curado "Look Blackout" cobre exatamente esse par) e passa a aparecer só em "Looks com essa peça"; só o Short Eclipse fica como parceira. Era exatamente a incoerência que o passo abaixo flagrou — o desconto aplicado no carrinho era o do curado, não os 20% da coleção que o bloco anunciava. Escolhido tamanho M no Top e na Legging, "Adicionar as duas" → 2 linhas novas com `conjunto_slot` `top-aura-blackout--legging-vertice-blackout#0#…`/`#1#…` (confirmado no Postgres); desconto aplicado foi o do curado "Look Blackout" (R$ 22,50 + R$ 22,50), não os 20% da coleção — esperado, mesmo efeito da ruling V2 (o par top+legging no carrinho é exatamente o conjunto do curado ativo).
  - **8 — PASSOU.** `/br/store`: selo "FORMA CONJUNTO" em Top Aura Blackout, Legging Vértice Blackout (via curado, ruling V4) e Short Eclipse Blackout; ausente no Macaquinho Nébula Blackout.
- **Antes do roteiro do dono:** o backend precisa do redeploy (`railway up`, rulings V2 e V6 — `catalogo-conjuntos.ts`) e a vitrine precisa do deploy do Vercel (branch `feat/conjunto-f3-vitrine` mergeada em `main`) — sem o redeploy do backend, produção continua mostrando o par de coleção duplicado ao lado de um curado com o mesmo conjunto de produtos.
- **Roteiro do dono em produção** (depois de ativar a regra padrão no Cockpit — aba Regras, editar e salvar com "Ativa" marcado):
  1. Abrir `/br/categories/conjuntos` — conferir curados e pares gerados listados, com preço "a partir de".
  2. Abrir a página de um conjunto (ex.: um par Top + Legging) — escolher cor/tamanho das duas peças, conferir o total com benefício no rodapé, clicar "Adicionar o conjunto".
  3. Abrir a PDP de um Top — conferir "Complete o conjunto" com as parceiras da coleção, escolher tamanho, "Adicionar as duas".
  4. Conferir o carrinho: linhas das peças adicionadas com preço já descontado (a linha "Benefício Conjunto" separada no resumo é F4, ainda não existe — por ora o desconto aparece no preço de cada linha).
  5. Voltar para `/br/store` e conferir o selo "Forma conjunto" nas peças elegíveis.
- **Limite conhecido novo:** capa de curado (`capa_url`) com host fora do allowlist do `next/image` (`images.remotePatterns`, `apps/storefront/next.config.js`) derruba a página Conjuntos inteira — só o uploader do Cockpit (Supabase Storage) é suportado; guarda de host fica para a F4.
- PENDÊNCIAS: (1) push/merge/deploy — dono faz `git push` (política do projeto); redeploy do backend (`railway up`) antes do deploy da vitrine; (2) roteiro de validação do dono acima, nenhum passo executado nesta task; (3) F4 (carrinho — etiquetas, linha "Benefício Conjunto" separada de cupom no resumo, gatilhos "Feche mais um conjunto", aviso de cupom) ainda não iniciada. Próxima ação: dono decide quando fazer o push/deploy e rodar o roteiro; depois, F4.

## 2026-09-09 — Benefício Conjunto F4 (Carrinho/checkout/pedido + Cockpit): código concluído + aceite local
- Spec: `docs/superpowers/specs/2026-09-08-beneficio-conjunto-design.md` (§7.5 "Entregue F4"; §10 linha F4; §11 itens 1–4, 9, 10) · SOP: `architecture/catalog.md` ("Carrinho e pedido — Fase F4, 2026-09"), `architecture/cockpit.md` §7 ("Detalhe do pedido (F4)") e `architecture/conjunto.md` §12/§13 · Pasta de execução: `.superpowers/sdd/2026-09-09-conjunto-f4-carrinho/` (briefs + relatórios das 6 tasks) · Branch `feat/conjunto-f4-carrinho`.
- Entrou (Tasks 1–5): módulo puro `apps/storefront/src/lib/util/carrinho-conjunto.ts` (`agruparDescontos`, `etiquetasDoCarrinho`, `etiquetaDoPedido`, `cuponsVisiveis`, `avisoCupom`, `montarGatilhos`, `tituloGatilho`, `slotGatilho`; 15 testes) + `getCarrinhoConjunto(cartId, countryCode, { comGatilhos })` em `lib/data/conjuntos.ts` (rota `oportunidades`, `no-store`, hidratação das candidatas) + `*items.adjustments` no `fields` de `retrieveCart`/`retrieveOrder`; etiqueta "Conjunto" na linha do carrinho/checkout/pedido; resumo desdobrado ("Benefício Conjunto" × "Cupom" × "Desconto" residual) em `cart-totals` e `order-summary`; `discount-code` esconde as promoções `CONJUNTO-*` e mostra o aviso de cupom; bloco "Feche mais um conjunto" (`modules/cart/components/gatilhos-conjunto/**`) com adição rápida em linha própria (`slotGatilho`) e tracking `conjunto_trigger_click` + `add_to_cart`; Cockpit — `lib/pedido-conjunto.ts` (5 testes), campos a mais em `medusaGetOrder` (`discount_total`, `items.metadata`, `items.adjustments.code/amount`) e badge + linhas nos Totais em `/pedidos`.
- **Sem mudança de backend nesta fase** — a F4 só consome o que a F1 já expõe (ajustes `CONJUNTO-<regra_id>`, marca `conjunto_desconto`, rotas `/store/conjuntos*`). Não há `railway up` novo por causa da F4; os redeploys pendentes continuam sendo os da F2 (`capa_url` nullable) e da F3 (rulings V2/V6).
- Rulings do controller registrados na spec (§7.5, verbatim): **1** etiquetas numeradas só onde existe carrinho (no pedido/Cockpit é só "Conjunto", por ajuste `CONJUNTO-` ou `metadata.conjunto_slot`); **2** resumo = ajustes `CONJUNTO-*` × demais ajustes de linha, resto na linha genérica "Desconto", cada linha só quando > 0; **3** lista de cupons esconde `CONJUNTO-*` e o aviso só aparece com cupom visível + benefício > 0; **4** gatilhos só na página do carrinho, até 3 candidatas, 1 unidade em linha própria, toast de sucesso/falha; **5** mini-cart não muda; **6** Cockpit só apresentação, sem rota nova e sem escrita; **7** se o checkout local não fechar, o §11 item 9 fica "dono valida no primeiro pedido real" — nunca criar pedido em produção; **8** sem teste de componente React; **9** **contagem Cockpit 39** (36 + 3), não 40 como dizia o plano; **10** (fix wave da revisão final, 2026-09-09) **Cockpit: "Itens" = `item_subtotal` + residual "Desconto"** — `item_subtotal` é o subtotal bruto das linhas, `total` da linha já vem líquido dos ajustes (usá-lo em "Itens" descontava Benefício Conjunto/Cupom duas vezes); `residualDesconto` (pura, testada) cobre a diferença entre `discount_total` e conjunto+cupom; **11** (fix wave da revisão final) **sem slug de categoria em prosa** — `montarGatilhos` descarta a oportunidade inteira quando não há nome resolvível para a categoria faltante, em vez de cair para o handle.
- **Suítes (após a fix wave da revisão final, 2026-09-09):** `npm test --workspace=apps/storefront` → **187 passed** (16 arquivos; era 186 antes da fix wave). `npm test --workspace=apps/cockpit` → **41 passed** (4 arquivos; era 39 antes). Backend (de `apps/backend`, `npm_config_script_shell="C:\Program Files\Git\usr\bin\bash.exe"`): `npm run test:integration:http` → **73 passed** (8 suítes), `npm run test:unit` → **30 passed** (3 suítes) — ambos inalterados, a F4 (e a fix wave) não tocaram o backend. `npx tsc -p apps/storefront --noEmit` e `npx tsc -p apps/cockpit --noEmit` → limpos.
- **Aceite spec §11 itens 1–4, 9, 10** (navegador via chrome-devtools, backend local semeado — `eclat_dev`/`eclat-pg-test`, regra padrão **ativa** só neste seed, ≠ produção; usuário admin local `admin@local.test`):

| Item | Cenário | Observado | Resultado |
|---|---|---|---|
| 1 | Top M + Short M (par shorts+tops = regra padrão; o par top+legging é coberto pelo curado, ruling V2), trocando o tipo da regra padrão pela Admin API | `menor_peca_percentual` 20 → **"Benefício Conjunto − R$ 31,80"** (total R$ 316,20); `menor_peca_valor` 3000 → **− R$ 30,00** (R$ 318,00); `total_percentual` 10 → **− R$ 34,80** (R$ 313,20); `total_valor` 5000 → **− R$ 50,00** (R$ 298,00). Subtotal R$ 348,00 e etiqueta "Conjunto" nas duas linhas em todos os quatro | **PASSOU** |
| 2a | 2× Top M + Legging M + Short M | Legging "Conjunto 1/2", Short "Conjunto 2/2", linha do Top (quantidade 2) "Conjunto 1/2 · Conjunto 2/2"; subtotal R$ 796,00, **"Benefício Conjunto − R$ 76,80"** = curado R$ 45,00 + par shorts+tops R$ 31,80; total R$ 719,20; sem seção de gatilhos (nenhuma oportunidade restante) | **PASSOU** |
| 2b | Top M + 2× Legging M | 1 conjunto: Top "Conjunto", Legging (quantidade 2) "Conjunto (1 de 2)", **"− R$ 45,00"**; gatilho "Mais uma peça de Tops da coleção Família Blackout fecha outro conjunto — 20% na peça de menor valor." com o card do Top Aura ("a partir de R$ 189,00"). Escolhido M e clicado "Adicionar" → linha nova (slot próprio), 2 conjuntos ("Conjunto 1/2"/"Conjunto 2/2" nos Tops, "Conjunto 1/2 · Conjunto 2/2" na Legging), **"− R$ 90,00"**, seção de gatilhos sumiu | **PASSOU** |
| 3 | Top M + Legging M + Macaquinho M, cupom `CUPOM10` (criado por `POST /admin/promotions`; o gancho `conjunto-cupom` acrescentou `items.conjunto_desconto eq nenhum`) | Subtotal R$ 747,00; **"Benefício Conjunto − R$ 45,00"**, **"Cupom − R$ 29,90"** (10% só do macaquinho de R$ 299,00), total R$ 672,10; aviso **"Cupom não se aplica a peças com Benefício Conjunto."** visível; lista de cupons só com `CUPOM10 (10%)` — nenhuma badge `CONJUNTO-*`. Macaquinho sem etiqueta. **Removendo o cupom** ("Remover cupom do pedido"): "Benefício Conjunto − R$ 45,00" **permanece**, "Cupom" e aviso somem, total volta a R$ 702,00 | **PASSOU** |
| 4 | Top M + Legging M + Short M | Curado (top+legging) formado primeiro: etiqueta "Conjunto" **só** no Top e na Legging, Short **sem** etiqueta; subtotal R$ 607,00, **"Benefício Conjunto − R$ 45,00"**, total R$ 562,00; gatilho "Mais uma peça de Tops da coleção Família Blackout fecha outro conjunto — 20% na peça de menor valor." (o Short precisa de outro Top) | **PASSOU** |
| 9 | `/br/checkout` e pedido concluído | Resumo do checkout com as **mesmas etiquetas** ("Conjunto" no Top e na Legging, nada no Short) e o mesmo desdobramento ("Benefício Conjunto − R$ 45,00", total R$ 562,00), sem gatilhos. Endereço preenchido e "Continuar para entrega" OK; **o checkout para no passo "Entrega › Forma de envio"**, que fica vazio: `GET /store/shipping-options?cart_id=` devolve `[]` porque o único `fulfillment_set`/`service_zone` do seed ("European Warehouse delivery"/"Europe") só tem geo zones `gb, de, dk, se, fr, es, it` — não há `br`. Confirmado também que `region_payment_provider` liga `pp_system_default` **só** à região Europe, ou seja, a região Brasil também não teria provedor de pagamento no passo seguinte. Nenhuma opção de frete/pagamento foi criada (nada em produção, nada no seed) | **PARCIAL** — metade checkout PASSOU; página do pedido, `/br/account/orders/details/<id>` e Cockpit › Pedidos ficam para o **dono validar no primeiro pedido real** (ruling 7) |
| 10 | Backend local parado com o storefront aberto, recarregar `/br/cart` | `GET /br/cart` respondeu **200** (duas vezes) e a página renderizou normalmente as 3 linhas (a partir do cache do carrinho), **zero etiquetas** e **sem** a seção "Feche mais um conjunto"; log do servidor da vitrine: `[conjuntos] falha ao buscar oportunidades do carrinho cart_… [TypeError: fetch failed] { [cause]: [AggregateError: ] { code: 'ECONNREFUSED' } }`; nenhum 500, nenhuma mensagem de erro na tela | **PASSOU** |

- **Estado do seed restaurado ao fim do aceite:** regra padrão de volta em `menor_peca_percentual` **20**, **ativa** (`PUT /admin/conjuntos/regras/creg_01M22V8BCA1VN6JPWCTMHHTARE`); `CUPOM10` deixado **inactive** (`POST /admin/promotions/promo_… { "status": "inactive" }`); exceção de coleção continua inativa; backend local reiniciado e respondendo em 9000. Nenhuma escrita em produção, nenhum `.env*` tocado.
- **Limites conhecidos novos (F4):** (1) a etiqueta do **pedido** (e do Cockpit) vem do ajuste `CONJUNTO-` ou de `metadata.conjunto_slot` — a unidade **não** descontada de uma regra "menor peça" adicionada pelo botão comum "Adicionar à sacola" da PDP fica sem etiqueta ali (no carrinho/checkout aparece, porque a etiqueta vem da rota `oportunidades`); (2) mini-cart sem etiqueta e gatilhos só no carrinho (rulings 5 e 4); (3) o **texto** do gatilho ("20% na peça de menor valor") vem de `GET /store/conjuntos`, cacheado com `revalidate: 300` — depois de trocar a regra no Cockpit ele pode ficar até 5 min desatualizado (observado no item 1); os **valores** do resumo acompanham na hora, porque vêm dos `adjustments`; (4) armadilha de validação local: `retrieveCart` usa `cache: "force-cache"` com tag revalidada só pelas Server Actions da própria vitrine — mexer no carrinho por fora (Store API direto) não invalida a entrada; é preciso uma mutação pela tela (quantidade, remover item, aplicar/remover cupom) para ver a mudança.
- **Antes do roteiro do dono:** push/merge da branch `feat/conjunto-f4-carrinho` em `main` (política do projeto: o dono faz o `git push`) + deploy da vitrine no Vercel. **A F4 não exige redeploy do backend**; os `railway up` pendentes continuam sendo os da F2 (`capa_url` nullable) e da F3 (rulings V2/V6), que precisam sair **antes** do deploy da vitrine.
- **Roteiro do dono em produção** (depois de ativar a regra padrão no Cockpit — aba Regras, editar e salvar com "Ativa" marcada):
  1. Adicionar um top + uma legging da mesma coleção → abrir `/br/cart`: as duas linhas com a etiqueta "Conjunto" e a linha **"Benefício Conjunto"** separada no resumo.
  2. Adicionar uma segunda legging → o bloco **"Feche mais um conjunto"** aparece no carrinho, com o texto da regra e os cards de candidata; escolher o tamanho e clicar "Adicionar" fecha o 2º conjunto (etiquetas viram "Conjunto 1/2"/"Conjunto 2/2").
  3. Aplicar um cupom (de itens) → conferir o **aviso** "Cupom não se aplica a peças com Benefício Conjunto" e a linha **"Cupom"** separada de "Benefício Conjunto"; remover o cupom e conferir que "Benefício Conjunto" continua.
  4. Concluir um **pedido de teste** (pagamento manual) → conferir `/br/order/<id>/confirmed` e `/br/account/orders/details/<id>` com a etiqueta "Conjunto" nas linhas e "Benefício Conjunto" separado no resumo; depois abrir **Cockpit › Pedidos**, abrir esse pedido e conferir o badge "Conjunto" no item e as linhas "Benefício Conjunto"/"Cupom" nos Totais. **Este passo fecha o §11 item 9** (ruling 7 — o aceite local não conseguiu concluir pedido por falta de frete/pagamento no seed).
- Contagens de teste desta fase: storefront **187**, cockpit **41**, backend **73** (integração HTTP) / **30** (unit) — atualizado após a fix wave da revisão final (2026-09-09; era storefront 186, cockpit 39).
- PENDÊNCIAS: (1) push/merge/deploy — dono faz `git push`; redeploy do backend (F2/F3) antes do deploy da vitrine; (2) ativar a regra padrão em produção; (3) roteiro de validação do dono acima, nenhum passo executado nesta task — o passo 4 é o que fecha o §11 item 9. Com isso, a Spec 2 (Benefício Conjunto) fica **concluída em código nas fases F0–F4**, restando só as validações do dono.
...
- **Decisões do dono** (registradas antes da implementação): (1) vídeo entra como **2º item** da galeria (logo após a 1ª foto, não no fim); (2) a estimativa de medidas a partir de altura/peso usa **heurística por IMC** (recalibrar depois com trocas reais — não é medição real); (3) desempate do tamanho recomendado escolhe o **maior tamanho**, e uma alternativa só aparece quando a medida cair a **≤ 4 cm** da faixa vizinha.
- VÍDEO: `product.metadata.youtube_id` (ID ou URL; preencher no Cockpit → Ficha técnica) → 2º item da galeria como facade (thumb + "Ver vídeo"); iframe youtube-nocookie só ao clique (autoplay/mute/loop/controls=0). `next.config.js` libera `img.youtube.com`. Evento dataLayer `video_play`. **Desvio já decidido em execução:** o componente de vídeo existe (`modules/products/components/product-video/index.tsx`, o facade em si), mas foi **integrado ao carrossel de galeria já existente** (`modules/products/components/image-gallery/index.tsx`, que monta os itens com `buildGalleryItems`); `variant-gallery` e `persona-gallery` apenas repassam `youtubeId`/`productHandle`/`personaMedia` adiante. Não foi criado nenhum componente de carrossel novo (fora de escopo, já decidido no plano).
- TAMANHO: `recommendSize` (função pura, Vitest, `lib/util/size-recommendation.ts`) compara busto/cintura/quadril com a tabela da categoria (`site_content.medidas`, escolhida por `getMeasureMap` + `getDeepestCategoryChain` + `pickMeasurements` em `modules/products/templates/index.tsx`, via caminho de handle da categoria mais profunda). Empate → maior tamanho; alternativa só se a medida estiver a ≤ 4 cm da faixa vizinha. Altura/peso só pré-preenchem (`estimateMeasurements`, heurística IMC — recalibrar com dados reais). Prefs locais em `eclat_prefs.medidas` (`EclatPrefs.medidas` em `lib/util/prefs-cookie.ts`: `altura_cm`, `peso_kg`, `busto`, `cintura`, `quadril` — só lidas no navegador; desde o fix `237449c` ficam fora do cookie espelho `eclat_prefs`, então nunca chegam ao servidor). Botão "Selecionar M" escolhe a variante (mesmo estado `options` do `ProductActions`, refletindo também no `MobileActions`). Evento `size_recommendation`.
- **Fix round da Task 5** (commit `237449c`): o recomendador só oferece tamanhos disponíveis na cor escolhida (`sizeAvail` do contexto); `medidas` excluídas do cookie espelho; validação altura/peso pelos limites de `estimateMeasurements` e medidas entre 40 e 200 cm; merge com medidas salvas antes; `setPrefs` com try/catch; `disabled` no fallback do Suspense.
- **Fix wave:** `desempate por ORDEM_TAMANHOS` (commit `3b66e55`) — o desempate "maior tamanho" comparava a ordem das linhas da tabela em vez da ordem canônica de tamanho; corrigido para usar `ORDEM_TAMANHOS` (lista própria `["P","M","G","GG"]`, alinhada à ordem de `SIZE_ORDER` de `catalog-facets`, sem PP/XG, que não existem nas tabelas de medida), independente de como a tabela foi cadastrada no Cockpit.
- `SizeGuide` da PDP (`modules/products/components/size-guide/index.tsx`) usa a **mesma tabela dinâmica** da recomendação: com `table` (vindo de `measureTable`, buscado uma única vez em `templates/index.tsx` e passado para `ProductActionsWrapper`/`ProductActions`/`SizeSelect`/`MobileActions`/`SizeRecommender` e para `SizeGuide`), renderiza as colunas/linhas da categoria (ex.: leggings mostra só Cintura/Quadril); sem tabela, cai no fallback fixo de 3 colunas (Busto/Cintura/Quadril, medidas padrão da marca) — regressão zero para produtos sem categoria com tabela (confirmado com Macaquinho Nébula Blackout).
- **Verificação no navegador** (backend local 9000 / storefront 8000): PDP de "Legging Vértice Blackout" (categoria leggings, tabela própria) → seção "Acerte o tamanho de primeira" mostra 2 colunas de medida (Cintura/Quadril, sem Busto) e o modal "Qual é o meu tamanho?" pede as duas mesmas medidas; preenchendo Cintura 70 / Quadril 96 o modal recomenda **M** com as **mesmas faixas** da tabela da página (68–74 cm / 94–100 cm) — mesma fonte confirmada; "Selecionar M" navega com `?v_id=` e marca "M" como tamanho selecionado, habilitando "Adicionar à sacola". PDP de "Macaquinho Nébula Blackout" (categoria sem tabela própria) → seção cai no fallback de 3 colunas (Busto/Cintura/Quadril) com os valores padrão da marca — regressão zero confirmada.
- **Follow-up pós-merge (2026-09-10, branch `fix/pdp-follow-up`, commits `3f48ba7`, `1f5227a`, item C):**
  (1) **Fora da tabela** — `recommendSize` devolve `foraDaTabela: true` quando alguma medida fica a mais de
  5 cm da faixa da linha vencedora ou a soma de distâncias passa de 10 cm; a UI troca o título por "Suas
  medidas ficam fora da nossa tabela", mostra "a mais próxima seria X", oferece "Ver tabela de medidas"
  (âncora `#medidas`) em vez de "Selecionar" e manda o evento `size_recommendation` com `fit:"fora_da_tabela"`.
  Ainda **sem link de WhatsApp**: a vitrine não tem URL/constante de WhatsApp — quando existir, virar link.
  (2) **`priority` por foto** na galeria — o slide de vídeo não consome mais uma das duas prioridades;
  sem vídeo o HTML volta a ser idêntico ao anterior à feature (2 primeiras fotos com preload/eager).
  (3) **Modal rolável** — o corpo dos passos ganhou wrapper `overflow-y-auto` dentro do painel (o `Modal`
  compartilhado, `overflow-y-hidden` + `max-h-[75vh]`, não foi tocado); em 375×667 o passo 3 rola até
  "Calcular novamente" e o aviso final.
  (4) Outros: desempate da **alternativa** por `ORDEM_TAMANHOS` (agora `= SIZE_ORDER`), `normalizarTamanho`
  nos dois lados da comparação rótulo × valor do Medusa, tamanho disponível mais próximo quando o
  recomendado está esgotado, `grid-cols` conforme as colunas da tabela, a11y (`role="status"`,
  `aria-describedby`, `Modal.Title` por passo) e `data-testid` `-mobile` no bottom sheet.
  (5) **Código morto removido** — `getMeasureTableForProduct` (`lib/data/measurements.ts`) e
  `lib/util/category-path.ts` (+ teste): a PDP usa `getMeasureMap` + `getDeepestCategoryChain`
  (`lib/data/category-path.ts`, arquivo diferente e vivo) + `pickMeasurements`.
  **Contagem final:** `npm test --workspace=apps/storefront` → **219 passed** (18 arquivos; 212 − 4 do
  `category-path.test.ts` + 11 novos de `size-recommendation.test.ts`); `npx tsc -p apps/storefront
  --noEmit` → limpo.
- FOLLOW-UPS: JSON-LD `VideoObject` (precisa de `uploadDate`); vídeo real de cada produto (asset); calibrar heurística IMC com trocas reais; considerar "Medidas da modelo" na ficha (DressOn faz); link de WhatsApp no resultado "fora da tabela" quando a vitrine tiver uma URL/constante de contato.
- **Suíte:** `npm test --workspace=apps/storefront` → **212 passed** na Task 6 (19 arquivos; contagem final após o follow-up: 219 em 18 arquivos; sem mudança nesta task — as Tasks 1–5 já deixaram `SizeGuide`/`templates/index.tsx` no estado final, a Task 6 só confirmou/validou). `npx tsc -p apps/storefront --noEmit` → limpo. **Problema conhecido de toolchain:** `npm run lint` (`next lint`) falha com `NOT SUPPORTED: option missingRefs` / `Cannot set properties of undefined (setting 'defaultMeta')` — erro pré-existente de configuração ajv/ESLint, não relacionado a este trabalho; os gates usados foram `tsc` + Vitest.

## 2026-09-10 — Lint do storefront de volta (ESLint CLI, flat config)
- Causa raiz do `npm run lint` quebrado nos dois apps: `overrides.ajv: "^8.0.0"` global no `package.json` da raiz (herdado da fundação, 5f10716) forçava o ajv 8 até no ESLint/`@eslint/eslintrc`, que exigem o 6 (`NOT SUPPORTED: option missingRefs`). Override removido; `ajv@6` volta aninhado sob `eslint` e `@eslint/eslintrc`; o ajv 8 da raiz continua (exigido por `umzug`/`@rushstack` no backend).
- Storefront: `next lint` (deprecado no Next 16) → `eslint .` com `eslint.config.mjs` (FlatCompat sobre `next/core-web-vitals` + `next/typescript`, mesma regra de `no-unused-vars` com prefixo `_`; `*.js` da raiz podem usar `require`; `*.test.ts` podem usar `any`). `.eslintrc.json` removido. 35 apontamentos corrigidos sem mudança de comportamento (args `_`, `catch` sem `any`, `c;` perdido em `check-env-variables.js`, `@ts-ignore` mortos, `require("path")` não usado, diretivas `eslint-disable` sem efeito). Resultado: 0 erros, 2 avisos de `react-hooks/exhaustive-deps` no checkout (deixados: mudar deps mudaria comportamento). Gates do storefront agora: tsc + Vitest + lint.
- Cockpit: o ESLint também voltou a rodar (`npm run lint --workspace=apps/cockpit`). Dos 41 apontamentos: `Param` de `configuracoes/page.tsx` hoisted para fora do componente (8× `react-hooks/static-components` — recriava o tipo a cada render), `setFRec` não usado removido, e `react-hooks/set-state-in-effect` (30×, padrão `carregar()` no mount, intencional) rebaixada a aviso na config até adotarmos o React Compiler. Resultado: 0 erros, 32 avisos.

## 2026-09-13 — Catálogo real: Blackout excluída, Lumière / Macaquinho Solaris (Telha) no ar ✅
- Fotos do ensaio (Garota da Foto, 10/09, modelo Camila, 140 originais) baixadas em `E:\Projetos\ECLAT\brand-assets\ensaio-camila-2026-09-10\`; o dono renomeia no padrão `Coleção <col> - Modelo <modelo> (<n>)-Cor <cor> .jpg`.
- Backup do catálogo anterior (8 produtos Blackout, 96 variantes, estoque) em `brand-assets/backup-catalogo-2026-09-13.json`; em seguida os 8 produtos foram EXCLUÍDOS em produção (pedido do dono). A coleção `familia-blackout` ficou vazia (ainda aparece no rodapé; nav a ignora).
- Novo importador `scripts/import-lumiere.py` (`--dry-run`, `--wipe`, `--modelo`): lê a pasta renomeada, otimiza (JPEG 2000px q85), hospeda em Supabase `site/products/<handle>/<cor>-NN.jpg`, cria produto (Tamanho × Cor, preço BRL decimal, categoria, coleção, canal, shipping profile), liga cada foto às variantes da cor (`/images/{id}/variants/batch`), estoque no CD Brasil, hex em `site_content.cores`. Idempotente: produto existente → refaz vínculos e estoque.
- Macaquinho Solaris (`macaquinho-solaris`, prod_01M2EC3R4664QEEMKS2HCTATVJ): coleção Lumière (`lumiere`, criada), categoria `macaquinhos`, cor Telha (#C27050 amostrado do tecido), tamanhos P/M/G (sem GG por decisão do dono), estoque P10/M20/G10, preço R$ 299,00 PROVISÓRIO (precificação pendente), 21 fotos. SKUs `ECL-MS-TEL-{P,M,G}`.
- Validação: `check-catalog-options.py` ✓; Store API com a chave Storefront devolve preço/estoque/21 imagens; vitrine local contra produção (PDP, seleção M, sacola R$ 299,00) ok. Lição: para rodar a vitrine local contra o backend Railway, criar `apps/storefront/.env.development.local` temporário (NEXT_PUBLIC_MEDUSA_BACKEND_URL + COMING_SOON_BYPASS=1) — o middleware Edge lê o env em runtime; apagar depois.
- Observação de UX: a galeria da PDP lista as 21 fotos numa coluna de miniaturas muito longa; avaliar limite (~8–10) ou curadoria por produto.
- Pendente: precificação; composição do tecido (`metadata.composicao` não preenchida); renomear/importar os outros 119 arquivos; decidir destino da coleção Blackout vazia; commit + push (dono).
- Porta VIP (13/09, pedido do dono): chave padrão trocada para `brilha01` em `lib/coming-soon.ts` (env `COMING_SOON_VIP_KEY` continua sobrepondo); `/clube` compara sem diferenciar maiúsculas/espaços e devolve `/em-breve?convite=invalido` em senha errada; página Em breve ganhou o campo "Tenho o convite do Clube" (form GET puro → `/clube`). Validado local: senha errada → mensagem; `BRILHA01 ` → loja liberada com o Solaris. Roteiro do Clube (D0) e architecture/ads.md atualizados com a nova senha. A chave só vale em produção após push + deploy da Vercel (dono).

## 2026-09-13 — Interruptor global "Minha ÉCLAT no site" (personas)
- Pedido do dono: ligar/desligar a função personas no site pelo Cockpit. Mecanismo: `site_content.personas = { ativo: boolean }` (mesma tabela/rota da Vitrine, `PUT /api/site-content/personas`); chave ausente ou inválida = ativo (comportamento anterior).
- Cockpit → Personas: switch "Minha ÉCLAT no site" no topo da página (`role="switch"`), carrega de `GET /api/site-content/personas` e salva no clique; o cadastro de personas/fotos continua editável mesmo desativada.
- Site: `personasAtivas()` em `lib/data/personas.ts` (via `getSiteContent`, revalidação ~30 s, nunca lança) + função pura `personasAtivasDe` (`lib/util/personas-flag.ts`, 2 testes). Desligada: o layout não renderiza o `Wizard`, o menu esconde "Minha ÉCLAT" e a PDP passa `personaMedia = []` (galeria padrão, mesmo com persona salva no navegador). Recomendador de tamanho e prefs de tamanho não mudam.
- Verificação: storefront 221 testes, tsc e lint limpos; cockpit tsc/lint (0 erros) e 41 testes. Validação visual do switch é do dono (login Supabase): desligar → esperar ~30 s → recarregar o site (wizard e "Minha ÉCLAT" somem; PDP com galeria padrão) → religar.
...
## 2026-09-13 — PDP: carrossel em todas as telas + ordem mobile
- Galeria (`image-gallery/index.tsx`): a faixa horizontal com scroll-snap passou a valer no desktop também (uma foto por vez, largura da coluna), com setas ‹ › sobre a foto (só desktop; no mobile arrasta) e bolinhas clicáveis em todas as telas. Animação própria por rAF (~260 ms, snap desligado durante o movimento, cancela a anterior, respeita `prefers-reduced-motion`): no Chrome, `scrollTo({ behavior: "smooth" })` dentro de contêiner `scroll-snap` era ignorado. Vídeo continua como slide; prioridade das 2 primeiras fotos mantida.
- Ordem no mobile (pedido do dono): coleção → nome → fotos → cor → tamanho → preço/botão → descrição → "+ Informações do produto" → "+ Envio e trocas" → resto. Implementada em `templates/index.tsx` com `order-*` no mobile e grid `300px | fotos | 300px` no desktop (cabeçalho e descrição na coluna esquerda, compra fixa à direita). `ProductInfo` foi dividido em `ProductHeader` e `ProductDescription` (`templates/product-info`).
- Validação: tsc, lint, 221 testes; navegador local — desktop 1440: 1 slide de 713 px visível, setas, 6 bolinhas; lógica de navegação conferida com scroll instantâneo (next 721 → 1442, prev, bolinha 6 → 3605 com "próxima" desabilitada); mobile 375: ordem dos blocos medida (coleção 219 → nome 259 → fotos 355 → cor 792 → tamanho 897 → preço 1045 → descrição 1318 → abas 1409/1466), sem overflow. A animação em si não é observável no Chrome de teste (janela em segundo plano pausa rAF) — conferir no navegador real.
...
- 13/09 (noite) — Macaquinho Solaris ganhou a cor **Grafite** (arquivo do dono: "Grafitti"; nome no site: Grafite, hex #312D2F amostrado do tecido). 6 fotos escolhidas de 12 (frente, 3/4, detalhe, lateral, costas, corpo inteiro), SKUs `ECL-MS-GRA-{P,M,G}`, estoque P10/M20/G10 (mesmo da Telha, a confirmar). Importador agora aceita as variações de nome que o dono usa ("- Cor", sem "Modelo", "(n)" antes ou depois), trata `ORDEM` como seleção exata, e completa produto existente: valores novos na opção Cor (`POST /products/{id}/options/{oid}` com lista completa), variantes novas por SKU (`POST /products/{id}/variants`), fotos novas (o update de `images` substitui a lista e exige `url` também nos itens existentes) e reordena a galeria. Validado: auditoria ✓, Store API, PDP ao vivo `?cor=Grafite` com swatch e 6 fotos; a descrição editada pelo dono no Cockpit foi preservada (o script não sobrescreve ficha de produto existente).
- 13/09 — cor renomeada de "Grafite" para **"Grafitti"** (decisão do dono): valor da opção Cor, título/opções das 3 variantes GRA, `site_content.cores` e `CORES` do importador. Auditoria ✓; PDP ao vivo `?cor=Grafitti` com swatch e 6 fotos (a página levou ~20 s para refletir por causa do cache ISR).

## 2026-09-13 (noite) — Lumière: Aurora e Orvalho + primeiros conjuntos montados ✅
- Fonte: pasta do Drive "EM PRODUÇÃO COLEÇÃO LUMINÁ" (fichas de produção Família Canelado Telha/Grafitti + Camiseta Raglan Dry). Grade 10P-15M-10G por modelo e cor (Solaris 10P-20M-10G). Tecido canelado 79% PA / 13% PES / 8% EL (385 g/m²), forro 91% PA / 9% EL, logo holográfico hotfix → `metadata.composicao` em todos (Solaris incluso, sem sobrescrever textos editados).
- Decisões do dono (13/09): só conjuntos MONTADOS dão desconto (regra padrão segue inativa, pares globais inertes); desconto 10% no total; conjunto vale por produto, qualquer cor; preço provisório R$ 169 em tops e shorts; nome "Top Aurora" (ficha dizia "Top Aura"); estoque = grade da ficha, sem GG; peças extras do ensaio (top decote redondo + legging/short com bolso lisos, conjunto bege, camiseta masculina sem foto) ficam para depois.
- Produtos criados (coleção Lumière, P/M/G, R$ 169): Top Aurora (Telha 7 fotos, Grafitti 6), Short Aurora (6/6), Top Orvalho (Grafitti 7, Telha 6 — fotos 043–061, top Telha com short Grafitti), Short Orvalho (Grafitti 6). **Short Orvalho Telha NÃO criado: não há foto no ensaio** (a peça foi produzida).
- Conjuntos (curados): `conjunto-aurora` = Top+Short Aurora, `conjunto-orvalho` = Top+Short Orvalho, `total_percentual` 10, capas em Supabase `site/conjuntos/`. Nome do curado SEM "Conjunto" (a vitrine prefixa no título, aviso e GA4).
- Validação em produção: carrinhos de teste pela Store API → conjunto mesma cor e cores misturadas: R$ 16,90 por peça (33,80); Top Aurora + Short Orvalho: 0; peça avulsa: 0. Vitrine ao vivo: página do conjunto R$ 338,00 → R$ 304,20 "10% sobre o conjunto"; PDP do top linka o conjunto; categoria Conjuntos lista os dois (cache de até ~15 min após criar/editar curado).
- Importador reestruturado: `GALERIA` (curadoria explícita por produto/cor, aceita fotos de conjunto e arquivos ainda não renomeados por prefixo), `CONJUNTOS` (cria/atualiza curados por handle), cor sem foto é pulada com aviso, metadata só preenche chaves ausentes, **estoque existente não é mais sobrescrito** (só com `--estoque`).

## 2026-09-13 — Manifesto da home: tipografia no mobile
- O texto do manifesto (site_content `home.manifesto`) era um único parágrafo serif grande e centralizado: no celular quebrava em linhas desiguais com palavra sozinha ("é."). Agora: faixa `areia/40` com filete terracota, cada frase em um bloco com `text-balance`, 22 px no mobile (36 px no desktop), segunda frase em itálico terracota; margem antes da seção seguinte. Validado a 390 px com o texto de produção (frase 1 em 2 linhas, frase 2 em 3, sem viúva). tsc, 225 testes.
...
- 13/09 (23h) — **Short Orvalho ganhou a cor Telha** e o Top Orvalho Telha trocou de galeria: 3 fotos do conjunto Orvalho Telha que o dono salvou em `G:\Meu Drive\ECLAT\ensaio-camila-2026-09-10` ("conjunto telha*.jpg"), copiadas para a pasta local como "Conjunto Orvalho - Cor Telha (1–3)". SKUs `ECL-SO-TEL-{P,M,G}`, estoque 10/15/10. Home (site_content): banner editorial de Conjuntos, linhas Tops/Shorts/Macaquinhos/Conjuntos com fotos do ensaio, destaque "Coleção Lumière", SEO sem Blackout (hero e manifesto já editados pelo dono ficaram intactos; backup em `brand-assets/backup-site-content-2026-09-13.json`).
- Importador: nome da foto no Storage leva hash do arquivo de origem (`<cor>-NN-<hash>.jpg`) para trocar curadoria sem cache velho; na atualização, fotos antigas geridas pelo script (`site/products/<handle>/`) saem da galeria, fotos de outra origem ficam. Obs.: a pasta do Drive (G:) é a cópia de trabalho do dono; a local (`brand-assets/`) é a fonte do importador — conferir as duas quando o dono disser "salvei na pasta".
- 13/09 (23h40) — **Textos oficiais do dono aplicados** (WhatsApp): descrição por modelo ("Nome. Frase.\n\nParágrafo"), `metadata.informacoes` por tipo (tops, shorts, macaquinhos — aba "Informações do produto"), `quem_sim`/`quem_nao` ("Feita pra você / Talvez não seja pra você"). FAQ reescrito só com fatos desses textos (saiu "sustentação média" do Top Aurora, que contradizia "estrutura maior"); chaves `dsb_*` removidas (a PDP não as exibe mais). Fonte única: `scripts/lumiere-textos.json` (o importador lê dela; não há mais copy no script). Backup dos textos anteriores em `brand-assets/backup-textos-produtos-2026-09-13.json`. No Short Orvalho, três finais de linha vieram cortados na captura e foram completados ("quer que reparem", "à corrida", "está com ele") — confirmar com o dono.

## 2026-09-14 — Banner da home em vídeo (desktop)
- Pedido do dono: dar movimento ao banner. As artes da Lumière tinham texto dentro da imagem (a IA deformaria as letras), então o vídeo nasce da arte SEM texto: os scripts `compor-banner-*.py` da pasta do projeto no Drive (`estudio/projetos/site-lumiere-banner`) foram reproduzidos pixel a pixel (fundo `noite-amanhecer-1`/`-vertical-1`, recorte `orvalho-frente`) e renderizados parando antes do texto. Vídeo desktop: Higgsfield Seedance 2.5, imagem→vídeo, 21:9, 5 s, 720p, sem som (32,5 créditos); movimento sutil (céu, luz, leve aproximação), rostos/mãos preservados. Loop de ida e volta de 10 s (1470×630, 764 KB) + capa = quadro inicial (54 KB), em `site/hero/` no Supabase Storage.
- Site: `lib/util/hero-media.ts` (puro, 6 testes) decide por tela vídeo (texto do site por cima) ou imagem (texto na arte); `hero-video.tsx` monta o vídeo só no navegador, só na tela certa (o do desktop nunca baixa no celular), sem "reduzir movimento" e sem economia de dados; capa aparece de imediato e o vídeo entra com fade. Logo, "Coleção Lumière", "FEITA PARA / brilhar.", parágrafo e botão desenhados pelo site nas proporções da arte (vw com mínimos). Mobile segue com a imagem até existir o vídeo vertical.
- Cockpit → Vitrine → Hero: bloco "Banner em vídeo" (URL do MP4 e capa por tela + textos). Conteúdo: `site_content.hero` ganhou `video_desktop_url`/`video_poster_desktop_url` (backup do valor anterior em `brand-assets/backup-site-content-hero-2026-09-14.json`); o site no ar ignora os campos até o deploy.
- Validado localmente: desktop 1440 com vídeo tocando e textos alinhados à arte; celular com a imagem, sem download de MP4. Pendente: vídeo mobile (Higgsfield desconectou da sessão), validação do Cockpit pelo dono.
- 2026-09-14 (depois): vídeo mobile gerado (Seedance 2.5, 3:4, 5 s, 32,5 créditos) e descartado. Com o texto do site por cima, o botão cobria o rosto da modelo: o 3:4 é recortado no banner 4:5 (modelos maiores e mais altas que na arte) e a aproximação da câmera sobe ainda mais as modelos; mesmo usando só os 2 primeiros segundos não coube. Campos do vídeo mobile retirados do `site_content.hero` (o celular segue com a imagem). Próximo passo proposto: arte mobile recomposta em 3:4 com as modelos mais baixas e câmera parada.
- 2026-09-14 (tarde): **vídeo mobile refeito e aprovado para uso.** Arte recomposta em 3:4 (1080×1440, `compor-banner-mobile-3x4-plate.py`: modelos ~300 px mais baixas, cabeças abaixo de 61% da altura) e gerada com câmera parada (Seedance 2.5, 3:4, 5 s, 32,5 créditos): céu da noite ao amanhecer, rostos e mãos intactos. Loop ida e volta de 10 s (834×1112, 629 KB) + capa (47 KB) em `site/hero/lumiere-mobile-3x4-loop.mp4` / `-poster.jpg`; cópias e original no Drive (`site-lumiere-banner/video/`). Caixa do banner mobile passou de 4:5 para **3:4** (sem recorte). Prévia a 390 px com o texto do site nos quadros mais escuro e mais claro: texto legível, botão termina em ~54% da altura, acima das cabeças. **Ligar só depois do deploy da vitrine** (em produção a caixa ainda é 4:5): gravar `video_mobile_url`/`video_poster_mobile_url` no `site_content.hero`.
- 2026-09-14 (depois do deploy de `3a85ced`): **vídeo mobile ligado em produção** com "pode aplicar" do dono. `site_content.hero` ganhou `video_mobile_url`/`video_poster_mobile_url` (backup do valor anterior em `brand-assets/backup-site-content-hero-2026-09-14-mobile.json`). Conferido em www.useeclat.com.br (via porta VIP) a 390 px: caixa 390×520, vídeo mobile tocando, só o MP4 do celular baixado, texto e botão acima das modelos.

## 2026-09-14 — Vitrine com um card por cor + conjuntos por cor ✅ (código; deploy da vitrine pendente do push)
- Opções 1 + 3 aprovadas pelo dono: um card por cor na listagem (mesmo produto no Medusa, zero mudança de estoque/conjuntos) e cards de conjunto por cor que abrem a página do conjunto já na cor. SOP: architecture/catalog.md "Vitrine com um card por cor".
- Código: `lib/util/listagem-cores.ts` (+10 testes), `catalog-facets.ts` (tamanho conta por cor, +1 teste), `lib/util/conjuntos.ts` (`montarCardsCuradoPorCor`, `coresComunsDisponiveis`, `hrefConjunto`, preço/foto por cor; +5 testes em `conjuntos-cores.test.ts`), `lib/data/listagem.ts` (interruptor `site_content.listagem`), `listProductsFiltered` → `entries`, `ProductListing` (grade por cor + conjuntos no fim), card (cor inicial + nome da cor), página/builder do conjunto (`?cor=`), destaque da home, GA4 `item_variant`. tsc, eslint e 247 testes ok.
- Validação local contra produção (porta 8010; a 8000 estava com o servidor de outra conversa): `/br/store` = 10 cards por cor + 4 conjuntos por cor (14 itens), "10 peças", facetas P/M/G 10 e Telha/Grafitti 5; `?cor=Telha` = 5 cards e sem conjuntos ("5 de 10 peças"); categoria Conjuntos = 4 cards por cor; `/conjuntos/conjunto-orvalho?cor=Telha` = as duas peças em Telha, só fotos Telha, total R$ 304,20; home = 10 cards por cor; console sem erros.
- Achado: servidores de dev de conversas diferentes na mesma pasta compartilham o cache de fetch do Next (`.next/cache`) — a primeira carga pode vir com catálogo antigo; recarregar após o revalidate.
- Pendente: push do dono → deploy da Vercel. Interruptor para desligar sem deploy: `site_content.listagem`.
- 14/09 (tarde) — **Fotos do Macaquinho Solaris Grafitti trocadas** pelas refeitas na cor real: 5 fotos "Coleção lumiére- macaquinho solaris grafitti*.jpg" que o dono salvou em `G:\Meu Drive\ECLAT\ensaio-camila-2026-09-10` (o link do Drive enviado não abria para a conta do Chrome local), copiadas para a pasta local como "… Cor Grafitti (101–105)". Galeria Grafitti = 105, 101, 104, 103, 102 (frente, 3/4, frente, costas, corpo inteiro); as 6 antigas saíram. Hex da cor Grafitti reamostrado para #3A363A (vale para todos os produtos Grafitti). Estoque e textos preservados.
- Correção: o importador agora atualiza a capa (`thumbnail`) ao refazer a galeria; capas antigas de Solaris, Top Orvalho e Short Orvalho apontavam para fotos que tinham saído da galeria e foram corrigidas.

## 2026-09-14 — Banner interativo no celular ("Lumière em Movimento")
- Pedido do dono: refazer no site o banner interativo criado em outra sessão (zip `banner-mobile-interativo-lumiere.zip` no Drive): céu do amanhecer com logo/"Coleção Lumière"/"Feita para brilhar."/parágrafo/botão e, abaixo, a modelo girando 360° — arrastar gira, 6 botões trocam a peça (Orvalho, Solaris, Aurora × Telha, Grafitti). Decisões do dono: opção 1 (componente do site com chave no Cockpit), **só no celular** (desktop segue com o vídeo), **botão só troca o giro** (sem link para a peça).
- Vídeos: os originais (720×1280, 8–14 s) tinham 2–3 quadros-chave (ex.: 0 / 4,4 / 12,7 s) → arrastar travaria. Recodificados com quadro-chave a cada 8 quadros, sem B-frames, crf 27: ficaram MENORES (22 MB → 13 MB no total; 1,7–3,2 MB cada). Capas = primeiro quadro. Em `site/hero/giro/` no Storage (+ `ceu-amanhecer.jpg`); servidor aceita Range (206).
- Site: `heroInterativo()` em `lib/util/hero-media.ts` (puro, 4 testes: desligado/sem peça válida → null e o celular segue com o vídeo; descarta peça incompleta; ids únicos) + `hero-interativo.tsx`. Carregamento: nada no desktop; no celular só a peça atual quando o palco chega perto da tela (as outras só ao tocar); economia de dados espera o 1º toque; "reduzir movimento" não toca sozinho. Arrasto: 320 px = uma volta, um seek por quadro de tela (rAF), pausa enquanto segura e retoma 0,9 s depois; setas do teclado giram. Troca: crossfade entre duas camadas quando o 1º quadro chega; erro de vídeo devolve o botão à peça na tela. Fora do link do banner (tem botões próprios).
- Cockpit → Vitrine → Hero: bloco "Banner interativo no celular" (chave, imagem do céu, lista de peças com nome/cor/hex/vídeo/capa, subir/descer/remover). Campos em `site_content.hero`: `interativo_mobile`, `interativo_ceu_url`, `interativo_pecas`.
- Validado localmente (vitrine local + proxy do Supabase que injeta a config, sem gravar produção), Chrome 390 px: 6 botões, 1ª peça tocando e só ela baixada; arrasto de 160 px levou o vídeo a 5,31 s = esperado; pausado segurando, volta a girar ao soltar; troca para Solaris Telha baixou só esse vídeo. Bug achado e corrigido: `canplay` dispara a cada seek e religava o vídeo durante o arrasto → tocar só no `loadeddata`. Desktop 1440: só o vídeo do desktop, nenhum MP4 de giro. tsc, lint, 251 testes.
- Obs. para o dono: no vídeo do Solaris Grafitti a peça parece cinza mais claro/azulado que as fotos refeitas (101–105). O Solaris Grafitti já estava cadastrado (sessão paralela, `cb68270`) com 5 fotos — não há 6ª foto na cor real.
- **Ligar só depois do deploy da vitrine**: gravar a config (`interativo-config.json` do scratchpad: 6 peças na ordem da arte) no `site_content.hero`.
- 2026-09-14 (após deploy de `52df566`): **banner interativo ligado em produção** com "pode aplicar" do dono — `site_content.hero` ganhou `interativo_mobile: true`, céu e as 6 peças (Orvalho Grafitti, Solaris Telha, Aurora Telha, Aurora Grafitti, Orvalho Telha, Solaris Grafitti); `video_mobile_url` mantido como reserva (desmarcar a chave no Cockpit volta ao vídeo). Backup em `brand-assets/backup-site-content-hero-2026-09-14-interativo.json`. Conferido em www.useeclat.com.br (porta VIP) a 390 px: 6 botões, só `orvalho-grafitti.mp4` baixado ao abrir, arrasto 160 px → 1,36 s = esperado, pausado segurando e retoma ao soltar, troca para Aurora Telha tocando. Dono decidiu manter o vídeo do Solaris Grafitti como está (cor mais clara que as fotos refeitas).
- 14/09 (fim da tarde) — **Qualidade das fotos dos cards de conjunto corrigida** (relato do dono). Diagnóstico medido no site ao vivo: as 75 fotos no Storage estão íntegras (≥ 1333x2000); o problema era o card de conjunto por cor, que trocou a capa por duas fatias estreitas e declarava `sizes` 12vw (versão de 256 px esticada para ~160x580 px: nitidez 66% em tela comum, 50–65% em retina/celular). Correção: capa por cor = 1ª foto da 1ª peça naquela cor (look completo) e `sizes` do card = 60vw/40vw/30vw. Validado local contra produção: 1 foto por card, nitidez 140–178% (desktop e celular, 2x/3x); cards de produto seguem 107–127%. Teste de `montarCardsCuradoPorCor` atualizado.

## 2026-09-14 — PDP: vídeos das peças (MP4 por cor) na galeria
- Pedido do dono: usar os vídeos da pasta "VÍDEOS BOLINHA" do Drive (6 MP4 9:16, ~9 s, com som, fundo escuro: Aurora/Orvalho/Solaris × Telha/Grafitti) na PDP. Home fora de escopo (banner interativo já usa esses vídeos). Angè/DressOn analisados em 07/09 continuam como referência.
- Nada baixado nem recodificado: os mesmos arquivos já estão em `site/hero/giro/` (recodificados, mudos, com capa) desde o banner interativo — a PDP aponta para eles.
- Site: `metadata.videos` = JSON cor → URL do MP4 ou {src, poster} (`parseProductVideos`, chave normalizada como as cores do catálogo); `videoForColor` (cor escolhida → vídeo dela; sem cor → o 1º; cor sem vídeo → nenhum, nunca o de outra cor); `resolveVideoSource` (MP4 da cor > `youtube_id`). `buildGalleryItems` recebe a fonte já resolvida; o id do slide de MP4 é a URL, então trocar de cor troca o slide (listKey do carrossel). `ProductVideo` toca `<video>` mudo em loop só ao clique, em `object-contain` sobre `#242328` (fundo do estúdio medido no 1º quadro): 9:16 inteiro dentro do slide 29:34, sem cortar cabeça/pernas. YouTube segue igual. `VariantGallery` resolve o vídeo pela cor do contexto; persona usa o 1º vídeo. 16 testes no `product-video.test.ts` (260 no total), tsc e lint limpos.
- Verificado localmente (vitrine 8001 → proxy descartável 9001 → Medusa de produção com `metadata.videos` injetado, sem gravar nada): Macaquinho Solaris em Telha → 2º slide com a capa `solaris-telha.jpg`; "Ver vídeo" toca `solaris-telha.mp4` (720×1280, mudo, loop, evento `video_play` com `video_provider: "mp4"`); trocar para Grafitti → slide vira `solaris-grafitti`; tops e shorts Aurora/Orvalho recebem o vídeo do conjunto. Console sem erros.
- `scripts/seed-videos-pdp.py` grava `metadata.videos` nos 5 produtos (top/short Aurora e Orvalho com o vídeo do conjunto; Macaquinho Solaris). Simulação por padrão; `--apply` só com "pode aplicar". **PENDENTE:** rodar com `--apply` após o deploy da vitrine (o site no ar ignora a chave até lá).
- 2026-09-14 (noite) — **Giro com a filmagem original (fundo real).** O dono achou o recorte artificial (cabelo em bloco, "colada" no preto) e mandou os originais (Drive "VÍDEOS BOLINHA": modelo filmada no estúdio sobre pano preto, 720×1280, 9–15 s). Tratamento só na luminosidade (`lutyuv` no Y: abaixo de 86 vira preto, rampa até 102, acima intacto) — o pano (Y ≤ 84, p95) some no preto e a roupa grafitti (Y ≥ 100) e a pele ficam como estão (118→116); cor intocada. Descartados: curva por canal (pele alaranjada) e vinheta (short do rodapé escurecia 30%). Recodificação igual à v1 (quadro-chave a cada 8, crf 27): 2,0–3,8 MB. Em `site/hero/giro-v2/` (+ capas) e no Drive `site-lumiere-banner/giro-v2-fundo-real/`. `site_content.hero.interativo_pecas` apontado para a v2 com "pode aplicar" (backup em `brand-assets/backup-site-content-hero-2026-09-14-giro-v1.json`); v1 continua no Storage. Conferido em produção: 6 peças na v2, tocando, troca ok. Resta uma dobra tênue do pano em alguns quadros (escurecer mais comeria a roupa grafitti).
- Arrasto: o seek dependia de `requestAnimationFrame`, que o Chrome reduz a 1/s em janela coberta/aba de fundo — no teste (janela do chrome-devtools coberta) só parte do arrasto era aplicada. Trocado por limite de tempo próprio (1 seek a cada 16 ms via `setTimeout`), sem depender do rAF. tsc e lint ok; validar em produção após o deploy.
- 2026-09-14 (19h) — **Banner interativo compactado** (dono: 3 telas de rolagem até as peças era demais; escolheu a opção 1). Céu de ~560 → ~320 px úteis (logo 42 px, título clamp 36–44 px, parágrafo 14 px, botão menor), palco 9:13 → **3:4** com `object-position: 50% 22%` (menos ar acima da cabeça, mais corpo), botões das peças em **uma linha rolável dentro do palco** (sobre a base, `touch-pan-x` + `stopPropagation` para o toque não virar arrasto; peça escolhida rola para o centro), dica "arraste" logo acima; saíram o rótulo "Escolha a peça" e a legenda. Bloco inteiro: 1370 → 860 px (céu+modelo+botões em pouco mais de uma tela a 390 px; o rosto da modelo já aparece na 1ª tela). Validado localmente com rAF a 1/s (janela coberta): arrasto exato (7,25 s esperado/obtido) — o fix de `setTimeout` funciona; toque no botão não arrasta; troca ok; linha rola (1109 px em 390). tsc, lint.
- Peças telha refeitas com corte de fundo mais forte (Y < 100 → preto, rampa até 118; nelas roupa ≥ 136 e pele ≥ 129, sem perda: braço 199/107/70 → 198/107/69): dobras do pano somem. Em `site/hero/giro-v3/` (+ Drive `giro-v3-telha-fundo-mais-preto/`). **Orvalho telha fica na v2**: o short grafitti dela cai 20% na v3 (103→82) — v3 só em Solaris telha e Aurora telha (`interativo-config-v3.json`). Troca em produção pendente de "pode aplicar".
- 14/09 (noite) — **Revisão de todas as fotos + galeria sem cortes** (pedido do dono). Causas: (1) caixa 29:34 da galeria cortava ~11% em cima/embaixo das fotos 2:3 → caixa 2:3 (mobile e desktop); (2) PDP sem `?cor=` mostrava todas as fotos de todas as cores (Solaris 26, Top Aurora 13) → cor padrão = 1ª cor com fotos + teto de 6 fotos (+ vídeo), também na página do conjunto; (3) curadoria: fotos com cabeça cortada (TA3, TA7, TO3–TO5, CO7, CO15–CO17, R046/R048/R049, MS25/MS28/MS38) e fotos que cortavam o short (CO3 no Short Orvalho Telha) saíram; 1ª foto de cada produto agora é focada na peça, com 4 recortes 2:3 (Top Aurora Grafitti, Short Aurora Grafitti, Top Orvalho Grafitti, Short Orvalho Telha). Todas as galerias com ≤ 6 fotos por cor (Short Orvalho Telha 3, Solaris Grafitti 5). Produção: galerias e capas atualizadas pelo importador, auditoria ✓, estoque/textos preservados. Validação local contra produção: SSR de todos os produtos/cores = ≤ 6 fotos + 1 vídeo e caixas 2:3; desktop 713x1070 e mobile 327x491 (1,500); troca de cor no mobile mantém 6 + vídeo; páginas de conjunto 12 e 9 fotos (6 por peça). tsc, eslint e 263 testes ok. Pendente: push do dono (vitrine).
- 14/09 (noite) — **SKUs das etiquetas aplicados** (planilha `ECLAT-SKU-Lumina.xlsx`): 30 variantes da Lumière renomeadas de `ECL-TA-TEL-P`… para `ECL-1001-P`… (REF por modelo+cor), com `barcode` = SKU e o SKU do item de estoque igual; estoque conferido contra a planilha (sem diferenças). Camiseta Masculina Raglan Dry (REF 2001/2002) está na planilha mas não no site (sem fotos). Composição corrigida por cor (Telha: forro mousse + logo off-white; Grafitti: forro grafitti + logo holográfico — antes dizia holográfico para tudo). Custos (`produto_custo`) são por variant_id: nada a migrar. Importador passa a gerar SKU/barcode pelas REFs. Backup: `brand-assets/backup-skus-2026-09-14.json`. Próximo: leitor de código de barras no Cockpit (fluxos a definir com o dono).
- 14/09 (noite) — **Leitor de código de barras: design proposto** em `docs/superpowers/specs/2026-09-14-leitor-codigo-barras-design.md`. Decisões do dono: 4 fluxos (conferência do pedido, entrada, inventário, venda presencial), Code 128 com o SKU, leitor USB/Bluetooth, "Lumière" também na etiqueta, histórico de movimentações (tabela `estoque_movimento`, migração 0010 — a aprovar), conferência avisa e permite despachar com motivo, venda presencial adiada até o Mercado Pago. Fases F0 (base) → F1 (conferência) → F2 (entrada) → F3 (inventário); F4 adiada. Aguardando aprovação do schema e da F0+F1 (Halt).

## 2026-09-14 — Leitor de código de barras: F0 (base) + F1 (conferência do pedido) em código ✅
- `apps/cockpit/lib/leitor.ts` (puro, 14 testes): `normalizarCodigo` (maiúsculas, espaços, hífen trocado por `'`/`/`/`_` no layout ABNT2×US, prefixo AIM), `ehSkuDaEtiqueta`, `resumoConferencia` (por SKU; faltando / a mais / fora do pedido / sem código), `avaliarLeitura` (sinal da tela), `validarConferencia` (servidor: recalcula com os itens reais; divergência só passa com motivo).
- `components/campo-leitor.tsx` (foco permanente sem roubar outro campo, Enter confirma, sinal verde/amarelo/vermelho + bip), `components/conferencia-pedido.tsx` (tabela esperado × bipado, peça errada identificada pelo catálogo, desfazer/recomeçar, motivo quando não fecha). `GET /api/leitor/variante?codigo=` → `medusaVariantePorCodigo` (`/admin/product-variants?sku=` e depois `barcode=`).
- Pedidos: gaveta com "Conferir peças" antes de Despachar; botão vira "Despachar mesmo assim" e só libera com motivo; rota de despacho valida e grava `metadata.conferencia` (status, itens esperado/bipado, fora do pedido, motivo, operador, data) ANTES do fulfillment; registro aparece no bloco Envio.
- **Bug antigo corrigido**: na Medusa 2.15.5 `items.quantity` volta null se `items.detail.quantity` não for pedido no `fields` — o detalhe do pedido (gaveta, despacho) e o DRE (COGS por item) recebiam quantidade null. Adicionado nas duas consultas.
...
- 2026-09-19 19:16 — pesos reais da Lumière gravados em produção (peso do PRODUTO, "pode gravar" do dono): Macaquinho Solaris 222 g, Top Aurora 137 g, Short Aurora 132 g, Top Orvalho 93 g, Short Orvalho 132 g. Embalagens corrigidas no código (saquinho 50 g, caixa 115 g). Com isso 1 peça sozinha cabe no Mini Envios (macaquinho 272 g com saquinho); 2 peças continuam em 5 cm de altura (confirmado pelo dono) e vão por PAC/SEDEX. Branch feat/frete-superfrete juntada na main (merge 86e4021) com a suíte verde: backend 387, vitrine 316, Cockpit 222.
- 2026-09-19 19:47 — **Frete SuperFrete no ar.** Deploy do backend (a2162aa2) + `ativar-superfrete.mjs --aplicar`: Mini Envios, PAC e SEDEX na zona Brasil; "Entrega Padrão" (R$ 24,90 fixo) desligada. Cotação real conferida por carrinho na Store API: BH 1 peça SEDEX R$ 14,90 (Mini/PAC escondidos por serem dominados), BH 2 peças frete GRÁTIS (≥ R$ 499 em MG), SP 1 peça Mini R$ 18,90 / PAC R$ 23,90 / SEDEX R$ 35,90, Manaus 1 peça Mini R$ 26,90. Falta: variáveis do remetente no Cockpit (etiqueta), saldo na carteira e pedido real de teste.
...
- Meias (dados da sócia, 2026-09-19): tamanhos 34-38 e 39-43; cores Cinza & Grafitti, Branco & Choco, Exército & Choco; nome "Meia Cano Médio"; bolinha de cor metade de cada cor; 10 pares por tamanho e cor (60 no total). Falta preço, nome e fotos para cadastrar. Óculos: a sócia ainda vai relacionar modelos e nomes.
- Testes: storefront 309/309 (28 arquivos); `tsc --noEmit` sem erro.

- Cadastro das meias: `scripts/cadastro-meias.py` (simulação por padrão; `--apply` grava). Simulação de 2026-09-19 em produção: produto `meia-cano-medio` ainda não existe, categoria Meias ativa, 6 variantes (`ECL-MEIA-<COR>-<TAM>`), 1 foto por cor (vista de perfil, pedido da sócia), bolinha de cor metade/metade via `swatch_url` em `site_content.cores`. Composição (95% poliamida, 3% elastano, 2% outras fibras) e peso (46 g o par) confirmados pela sócia; **APLICADO em produção em 2026-09-19** com "pode aplicar" da sócia, depois do push para `main` e do deploy da vitrine: produto `meia-cano-medio` publicado, 6 variantes com 10 pares cada no CD Brasil, 3 cores novas em `site_content.cores` com `swatch_url`. PDP conferida em produção: sem aba de medidas e sem link "Guia de medidas".
...
## 2026-09-19 (noite) — Cartão recusado por "alto risco": o que faltava no nosso payload
- Sintoma: Pix pago em produção (pedido #10, R$ 49,80, depois estornado), mas TRÊS cartões seguidos (dois cartões, duas pessoas diferentes) recusados com `cc_rejected_high_risk`.
- Diagnóstico pela API do Mercado Pago (`/v1/payments/search`): nas cobranças de cartão o pagador chegava VAZIO (`email`, `identification`, `first_name`, `phone` todos `null` — no Pix vinham preenchidos) e `additional_info.tracking_id` mostrava **`security:none`**, ou seja, sem o identificador do aparelho. Conta do Mercado Pago já estava sem pendências (endereço preenchido, Pix ativo, conta jurídica confirmada pelo suporte).
- Correção (backend `src/modules/mercadopago`): `montarPayer` passa a mandar `last_name`, `phone {area_code, number}` e `address` (rua, número, bairro, cidade, UF, CEP, país) além de e-mail/nome/CPF; payload ganha `items` (título, quantidade, preço unitário, SKU); `criarOrder` aceita o id do aparelho e envia no header **`X-Meli-Session-Id`** (nome confirmado na doc oficial). Campo ausente é omitido — string vazia conta contra na análise de risco.
- Correção (vitrine): `lib/util/pagamento-dados.ts` (puro, 4 testes) monta sobrenome/telefone/endereço/itens a partir do CARRINHO no servidor; `lib/util/device-id-mercadopago.ts` lê `MP_DEVICE_SESSION_ID`; o passo de pagamento carrega `https://www.mercadopago.com/v2/security.js` (`view="checkout"`) para Pix e cartão. Conferido no navegador: a variável nasce como `armor.…` poucos segundos após o script carregar (lemos só na hora de pagar).
- Testes: backend 390, vitrine 324, lint limpo. `tsc` do backend segue com os mesmos 21 erros pré-existentes do painel antigo (nenhum nos arquivos do Mercado Pago).
- Não garante aprovação: quem decide é o antifraude do Mercado Pago. Em paralelo, o dono abriu chamado citando os pagamentos recusados `178962873355` e `179948847088`.

## 2026-09-19 — Redesenho da sacola + base de cores da marca no fluxo de compra (etapas 1 e 2) ✅ código
- Pedido do dono: sacola, menu e checkout "simples e sem estilo". Diagnóstico: fluxo ainda com o tema padrão da Medusa. Plano em 4 etapas aprovado; feitas a 1 (base) e a 2 (sacola). SOP: architecture/catalog.md "Identidade visual do fluxo de compra". Skills usadas: frontend-design, mobile-principles.
- Branch `feat/design-sacola-checkout` em worktree próprio (`../eclat-wt-design`, node_modules por junção ao clone principal, `next dev` sem turbopack na porta 8020) para não misturar com as alterações não commitadas do Mercado Pago no clone principal.
- Validação local contra produção (carrinho de teste criado pela Store API): celular 375px sem rolagem lateral, alvos de toque 44px, barra fixa com total, fotos carregando; desktop com nota à direita; checkout já sem azul/preto (0 elementos). Clique real: + (1→2, total 817→976), − (volta), Remover (3→2 linhas, conjunto R$ 299). tsc, eslint e 320 testes ok.
- Achados de ambiente: (1) cookies de `localhost` são compartilhados entre portas — um `_medusa_cart_id` httpOnly de outra sessão impede trocar de carrinho por JS; usar `http://127.0.0.1:<porta>` dá um pote de cookies limpo. (2) Com o painel do navegador em segundo plano a hidratação dos blocos em Suspense atrasa: `.click()` por JS não dispara nada, clique real (ferramenta computer) hidrata e funciona.
- 2026-09-19 22:55 — **Regras reais da Orders API descobertas por sondagem em produção** (o primeiro deploy da correção quebrou o Pix com HTTP 400): `payer.address.country` e `items[].unit_measure` NÃO são aceitos (`unsupported_properties`), e a SOMA dos itens precisa bater com `total_amount` (`order_items_total_amount_mismatch`). Aceitos: `complement` no endereço, `type`/`external_code`/`description` no item, item com valor NEGATIVO (desconto) e `quantity × unit_price`. Correção: backend sem `country`/`unit_measure` e só manda `items` quando a soma fecha (item é opcional — nunca derruba o pagamento); vitrine manda o frete como item e o desconto como item negativo. Testes: backend 391, vitrine 326.
- 2026-09-20 06:40 — **Pix confirmado de volta** depois do deploy b57b5e2f (23:22): teste pelo servidor gerou QR e sessão `pending` (order ORD01M2Z2VAANVEV99T99EYZSWJSG). Duas correções do diagnóstico de ontem, ambas verificadas contra a API real: (1) o `payer` vazio em `/v1/payments/search` é como o Mercado Pago EXIBE pagamento pendente/recusado — não prova payload incompleto (o Pix aprovado de 19/09 mostra o payer mascarado); (2) o header `X-Meli-Session-Id` **não tem efeito na Orders API** — duas orders iguais, uma com device id REAL do `security.js` e outra sem, ficaram as duas com `security:none` no `tracking_id`. O código do device id fica (inofensivo), mas a alavanca real que resta para o cartão é **3-D Secure** (`transaction_security` na Orders API) + o chamado aberto no Mercado Pago. Detalhes em `contexto-claude/eclat-pagamento-mercadopago.md`.
- 2026-09-20 08:07 — **Miniatura do link trocada** ("pode aplicar" do dono): a prévia do site ainda mostrava a arte da coleção de teste "Família Blackout" (`site/banners/og-home.jpg`). Nova imagem 1200×630 feita a partir da arte atual da Lumière (`banner-site-desktop-lumiere.png`, 1920×860) com faixa `#0B0B10` em cima e embaixo — cortar as laterais comia a logo/título ou partia a terceira modelo. Em `site/banners/og-home-lumiere-2026-09.jpg` (95 KB; nome novo por causa do cache de 1 ano) e cópia em `docs/design/banners/og-home-lumiere-1200x630.jpg`; `site_content.seo.home.og_image_url` atualizado (backup em `brand-assets/backup-seo-home-2026-09-20.json`) e confirmado no HTML de produção. Obs.: o WhatsApp guarda a prévia antiga por URL — compartilhar com `?v=2` ou recarregar no debugger do Facebook. Só a home define imagem própria; páginas como loja e coleções seguem sem imagem padrão (mudança de código, pendente de decisão do dono).

## 2026-09-20 — Menu do celular redesenhado
- Pedido do dono: "o menu no mobile continua sem design e feio". Era o painel do starter do Medusa: fundo escuro translúcido, ícones genéricos e um quadrado cinza com a INICIAL da categoria (espaço reservado para foto que as categorias não têm).
- Agora: painel claro (`eclat-luz`) em tela cheia (88% da largura, máx. 400 px), fundo da página escurecido em `grafite/45`; topo com a marca e um botão "Fechar" escrito; categorias em serifa 22 px, cada linha com um fio terracota que cresce da esquerda em cascata (40 ms entre linhas, desligado em "reduzir movimento"); subcategorias num bloco `areia/40` com "Ver tudo de X"; grupos nomeados em maiúsculas terracota ("Peças", "Coleções", "Sua conta"); rodapé com país e direitos. Foco de teclado visível em terracota.
- "Toda a loja" saiu de "Sua conta" e entrou no fim de "Peças" (é navegação, não conta).
- Arquivos: `modules/layout/components/side-menu/index.tsx` (reescrito) e `styles/globals.css` (animação `menu-fio`).
- Conferido a 375/390 px com os dados de PRODUÇÃO (vitrine local apontando para o Railway, porta 8100 — a 8000 estava ocupada por outra sessão): grupos, acordeão de Acessórios (Óculos, Meias, Ver tudo), rodapé e scrim. tsc, lint e 326 testes.
- Achado em passagem: a página `/account` tem trecho em inglês ("Got questions?" / "You can find frequently asked questions and answers…") — corrigir depois.

## 2026-09-20 — Pedido mínimo, cupom ERIKA20 e o maior desconto por peça
- **Pedido mínimo R$ 150 em peças** (decisão do dono): `apps/backend/src/modules/pedido-minimo/regra.ts` (7 testes) + gancho `validate` do `completeCartWorkflow` (o servidor recusa o fechamento, não dá para furar pela Store API) e, na vitrine, `lib/util/pedido-minimo.ts` (5 testes) com aviso na sacola e botão travado (desktop e barra fixa do celular). A conta é o preço CHEIO das peças (preço × quantidade), sem descontos e sem frete — cupom não derruba o mínimo, frete não ajuda a alcançá-lo.
- **Maior desconto por peça** (decisão do dono: "no caso de conjunto o sistema separa as peças e aplica o desconto maior"): `marcarContexto` agora recebe os cupons PERCENTUAIS do carrinho e, unidade a unidade, compara o desconto do conjunto com o do cupom; ganhando o cupom, a unidade sai como `nenhum` (o cupom a alcança e o conjunto não a desconta). Empate fica com o conjunto (é automático). Peça marcada `conjunto` (regra "menor peça", sem desconto próprio) passa a ficar livre quando há cupom — antes ficava bloqueada para os dois. Cupom de VALOR FIXO fica de fora da comparação (o motor reparte `across`; "quanto daria nesta peça" seria conta circular) e segue alcançando só as peças fora de conjunto. 8 testes novos em `maior-desconto.unit.spec.ts`.
- **Cupom**: `scripts/cupom.mjs` (simulação por padrão) cria campanha com limite **por cliente** (`budget.use_by_attribute`, mesmo desenho do Clube) + promoção percentual de ITENS (nunca alcança frete). ERIKA20 = 20%, 1x por cliente; o mínimo de R$ 150 do cupom é o mesmo mínimo da loja, então não há regra separada.
- Testes: backend 406, vitrine 331. **Pendente**: deploy do backend ANTES de criar o cupom (a regra do maior desconto mora no backend) e, depois, `node scripts/cupom.mjs --codigo ERIKA20 --percentual 20 --por-cliente 1 --aplicar` com "pode aplicar" do dono.
- **Entrega por aplicativo** (decisão do dono: Betim + RMBH, retirada combinada por WhatsApp depois do pagamento): módulo `apps/backend/src/modules/entrega-app` — provider de fulfillment próprio, frete R$ 0 dentro da área e recusa (`NOT_ALLOWED`) fora dela, o que faz a opção sumir sozinha da vitrine (mesmo caminho do provider da SuperFrete). Área por FAIXA DE CEP (o nome da cidade vem do formulário e não dá para confiar): BH, Contagem, Ibirité, Betim, Santa Luzia/Vespasiano, Ribeirão das Neves, Nova Lima/Sabará/Caeté; ampliável por `ENTREGA_APP_CEPS` sem deploy. O aceite da cliente é exigido pelo servidor (`validateFulfillmentData` recusa sem `data.aceite === true`) e o pedido guarda o texto aceito e a hora. 10 testes.
- Vitrine: caixa "Estou de acordo: eu chamo e pago o transporte" dentro da opção; enquanto ela não é marcada, a opção fica desabilitada; ao escolher, o aceite vai junto (`lib/util/entrega-app.ts`, 2 testes). Cockpit: aviso na tela de despacho ("não compre etiqueta", combine por WhatsApp), data do aceite e botão de etiqueta desabilitado nesse pedido (`lib/entrega-app.ts`, 5 testes).
- `apps/backend/ativar-entrega-app.mjs` (simulação por padrão) vincula o provider ao CD Brasil e cria/liga a opção na zona Brasil; `--aplicar --desfazer` tira da loja.
- Testes: backend 416, Cockpit 227, vitrine 333. **Pendente**: push + `railway up` (o provider precisa existir no ar) e então, com "pode aplicar": `node apps/backend/ativar-entrega-app.mjs --aplicar` e `node scripts/cupom.mjs --codigo ERIKA20 --percentual 20 --por-cliente 1 --aplicar`.
- 2026-09-20 17:40 — **Cupom dando erro na sacola: duas causas, as duas corrigidas.** (1) O ERIKA20 tem limite POR CLIENTE (`budget.use_by_attribute customer_id`) e o Medusa recusa quando o carrinho ainda não tem cliente — na sacola não há e-mail, então ele só entra depois do e-mail do checkout (ou login). Confirmado na API: sem e-mail → `Attribute value for "customer_id" is required by promotion campaing budget`; com e-mail → aceito (o Medusa cria o cliente convidado). (2) `applyPromotions` ESTOURAVA dentro de uma ação de servidor, e o Next mostrava "An error occurred in the Server Components render…" na tela da cliente. Agora a ação devolve `{ ok }` ou `{ ok: false, mensagem }` e `lib/util/erro-cupom.ts` (4 testes) traduz: limite por cliente → "entre na sua conta ou informe o e-mail no checkout"; código errado → "Cupom X inválido ou expirado"; limite esgotado → "já foi usado". Vitrine: 337 testes.
