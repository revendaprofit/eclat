...
- **Suítes:** storefront `npm test --workspace=apps/storefront` → **161 passed** (14 arquivos). Cockpit `npm test --workspace=apps/cockpit` → **36 passed** (3 arquivos). Backend (de `apps/backend`, `npm_config_script_shell="C:\Program Files\Git\usr\bin\bash.exe"`): `npm run test:integration:http` → **72 passed** (8 suítes, container `eclat-pg-test`); `npm run test:unit` → **30 passed** (3 suítes). `npx tsc -p apps/storefront --noEmit`, `npx tsc -p apps/cockpit --noEmit` e `npx tsc -p apps/backend --noEmit 2>&1 | grep "error TS" | grep -v cockpit/page.tsx` → todos limpos.
