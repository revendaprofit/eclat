...
- **Suítes:** `npm test --workspace=apps/storefront` → **186 passed** (16 arquivos). `npm test --workspace=apps/cockpit` → **39 passed** (4 arquivos). Backend (de `apps/backend`, `npm_config_script_shell="C:\Program Files\Git\usr\bin\bash.exe"`): `npm run test:integration:http` → **73 passed** (8 suítes), `npm run test:unit` → **30 passed** (3 suítes) — ambos inalterados, a F4 não tocou o backend. `npx tsc -p apps/storefront --noEmit` e `npx tsc -p apps/cockpit --noEmit` → limpos.
...
- Contagens de teste desta fase: storefront **186**, cockpit **39**, backend **73** (integração HTTP) / **30** (unit).
