...
- Rulings do controller registrados na spec (verbatim, ver §7/§12 amendados): **V1** "a partir de" no card + `view_item_list` só dos cards inicialmente visíveis + fallback de thumbnail; **V2** par ofuscado por curado (acima); **V3/V5** "Adicionar o conjunto"/"Adicionar as duas" resumíveis após falha parcial — progresso atrelado à seleção corrente (sem `useEffect` de reset), erro nomeia a peça, seletores travados durante a adição; **V4** selo "Forma conjunto" também para peça elegível só via curado ativo (fecha a ofuscação que o V2 introduzia na elegibilidade).
