...
- VÍDEO: `product.metadata.youtube_id` (ID ou URL; preencher no Cockpit → Ficha técnica) → 2º item da galeria como facade (thumb + "Ver vídeo"); iframe youtube-nocookie só ao clique (autoplay/mute/loop/controls=0). `next.config.js` libera `img.youtube.com`. Evento dataLayer `video_play`. **Desvio já decidido em execução:** não existe um componente de vídeo separado — o facade foi mesclado dentro do carrossel de galeria já existente (`variant-gallery`), que apenas repassa `youtubeId`/`personaMedia` adiante; não foi criado nenhum componente de carrossel novo (fora de escopo, já decidido no plano).
- TAMANHO: `recommendSize` (função pura, Vitest, `lib/util/size-recommendation.ts`) compara busto/cintura/quadril com a tabela da categoria (`site_content.medidas`, escolhida por `getMeasureTableForProduct`/`pickMeasurements` via caminho de handle da categoria mais profunda). Empate → maior tamanho; alternativa só se a medida estiver a ≤ 4 cm da faixa vizinha. Altura/peso só pré-preenchem (`estimateMeasurements`, heurística IMC — recalibrar com dados reais). Prefs locais em `eclat_prefs.medidas` (`EclatPrefs.medidas` em `lib/util/prefs-cookie.ts`: `altura_cm`, `peso_kg`, `busto`, `cintura`, `quadril` — só lidas no navegador; desde o fix `237449c` ficam fora do cookie espelho `eclat_prefs`, então nunca chegam ao servidor). Botão "Selecionar M" escolhe a variante (mesmo estado `options` do `ProductActions`, refletindo também no `MobileActions`). Evento `size_recommendation`.
...
- FOLLOW-UPS: JSON-LD `VideoObject` (precisa de `uploadDate`); vídeo real de cada produto (asset); calibrar heurística IMC com trocas reais; considerar "Medidas da modelo" na ficha (DressOn faz).
