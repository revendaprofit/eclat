...
- Backend tocado nesta branch (Task 5): `apps/backend/src/api/middlewares.ts` — `capa_url` agora `.nullable()` nos schemas de criar/editar curado, para o Cockpit poder enviar `capa_url: null` (limpar capa) sem `400`. +2 testes de integração — suíte de integração do módulo (`conjunto-*.spec.ts`) em **70** no total.
...
- **Roteiro de validação do dono** (Cockpit em produção, login do dono — nenhum destes passos foi executado nesta task):
  ...
  4. Aba Conjuntos curados: criar "Look teste" com 2 produtos publicados, capa, tipo "R$ sobre o total" 40,00; ver na lista; arrastar para reordenar; excluir.
