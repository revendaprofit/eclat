...
- Cadastro das meias: `scripts/cadastro-meias.py` (simulação por padrão; `--apply` grava). Simulação de 2026-09-19 em produção: produto `meia-cano-medio` ainda não existe, categoria Meias ativa, 6 variantes (`ECL-MEIA-<COR>-<TAM>`), 1 foto por cor (vista de perfil, pedido da sócia), bolinha de cor metade/metade via `swatch_url` em `site_content.cores`. Composição (95% poliamida, 3% elastano, 2% outras fibras) e peso (46 g o par) confirmados pela sócia; falta só o "pode aplicar".
