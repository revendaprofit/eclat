...
3. **Uma mensagem só, completa, e um remetente só:** é a mesma mensagem de despacho de hoje (número do pedido + código + link de acompanhamento), não um segundo aviso. **Quem envia é sempre o backend** (função `tentarAvisoDeDespacho`), chamado de três lugares: pelo Cockpit logo depois do despacho (se o código já existir, sai na hora), pelo webhook `order.generated` e pela verificação de 5 minutos. A função relê o pedido e usa uma trava antes de enviar, para dois chamadores simultâneos nunca mandarem duas vezes. O Cockpit **não** manda mais esse WhatsApp no caminho da etiqueta — assim o texto existe num lugar só (a lição dos pesos das embalagens, que divergiram quando havia duas cópias). *(Refinamento do controlador em 2026-09-21; não muda o que a cliente recebe.)*
3a. **Interruptor de implantação no Cockpit, `SUPERFRETE_AVISO_PELO_BACKEND`** (desligado por padrão). Desligado = comportamento anterior: o Cockpit manda o WhatsApp na hora, com o que tiver. Ligado = segura o aviso e deixa o backend enviar. Motivo: a Vercel publica o Cockpit sozinha no push, mas o backend só vai ao ar com `railway up` manual; sem o interruptor, "Cockpit novo com backend velho" deixaria toda cliente sem aviso. Ordem segura: `railway up` → conferir que o backend envia → ligar a variável na Vercel → redeploy.
