...
**Confirmado F3 (2026-09-09, Task 4 de `feat/conjunto-f3-vitrine`):** testado empiricamente contra o Medusa local (2.15.5) que `metadata.conjunto_slot` **não funde linhas** — o Medusa não trata `metadata` como parte da identidade de merge só quando ausente; com um valor de `conjunto_slot` presente (mesmo com a mesma variante), cada `addToCart` sempre cria uma linha nova. Verificado na página do conjunto (`/conjuntos/[handle]`): adicionar o mesmo conjunto duas vezes seguidas (mesma seleção de tamanho/cor) resultou em **2 → 4 linhas** no carrinho, nunca 2 linhas com quantidade 2 — cada uma com `conjunto_slot` distinto (o sufixo `#<timestamp>` garante isso mesmo quando o índice do slot se repete). Isso fecha o "limite conhecido" da F1 (§12, achado I2) **na prática, só para os fluxos dedicados** — página do conjunto e bloco "Complete o conjunto" da PDP —, porque toda peça adicionada por esses fluxos vai sempre para uma linha de quantidade 1, própria, então nunca compartilha `item.id` com outra unidade de marca diferente (cupom incluso). **Uma peça adicionada pelo botão comum "Adicionar à sacola" da PDP continua sem esse metadata** (nenhuma mudança nesta fase no fluxo padrão de adição) — se essa linha comum, sem `conjunto_slot`, acabar parcialmente marcada por um conjunto (ex.: a cliente já tinha 2 unidades do mesmo top no carrinho e só 1 delas fecha um conjunto depois), o limite I2 (linha dividida compartilha a base de desconto com um cupom) **continua valendo** para ela — não foi eliminado de forma geral, só contornado nos pontos de entrada que a F3 controla.
...
**Entregue F4 (2026-09-09)** — branch `feat/conjunto-f4-carrinho`; sem mudança de backend. Módulo puro `apps/storefront/src/lib/util/carrinho-conjunto.ts` (15 testes) + `getCarrinhoConjunto` em `lib/data/conjuntos.ts` (rota `oportunidades`, `no-store`) + `*items.adjustments` no `fields` de `retrieveCart`/`retrieveOrder`; espelho no Cockpit em `apps/cockpit/lib/pedido-conjunto.ts` (5 testes). Detalhe: `architecture/catalog.md`, "Carrinho e pedido (Fase F4, 2026-09)" e `architecture/cockpit.md` §7 ("Detalhe do pedido (F4)"). Após a fix wave da revisão final (2026-09-09): storefront **187** testes, cockpit **41** testes. Rulings do controller desta fase:
...
2. **Resumo:** "Benefício Conjunto" = soma dos ajustes `CONJUNTO-*` das linhas; "Cupom" = soma dos demais ajustes de linha; ajustes de método de envio não entram; se a base do residual > a soma dos dois, a diferença aparece na linha genérica "Desconto". Base do residual: `discount_subtotal` no storefront (`CartTotals` e `order-summary`, com *fallback* para `discount_total` quando o campo não vem no objeto); `discount_total` no Cockpit, por ser o único campo de desconto do pedido na Admin API que a tela já tinha. Cada linha só aparece quando > 0.
...
10. **Cockpit: "Itens" = `item_subtotal` + residual "Desconto"** (fix wave da revisão final, 2026-09-09): `item_subtotal` é o subtotal bruto das linhas (sem desconto/imposto); o `total` da linha já vem líquido dos ajustes, então usá-lo em "Itens" descontava Benefício Conjunto/Cupom duas vezes. `residualDesconto(discountTotal, grupos)`, pura e testada, cobre a diferença entre `discount_total` e a soma de conjunto+cupom (ex.: ajuste de frete), exibida como "Desconto" só quando > 0.
11. **Sem slug de categoria em prosa** (fix wave da revisão final, 2026-09-09): `montarGatilhos` descarta a oportunidade inteira quando `nomesCategorias` não tem o handle da categoria faltante, em vez de cair para o handle — nunca mostrar um slug técnico no título do gatilho.
...
- **Linha dividida compartilha a base de desconto entre conjunto e cupom** (mesmo `item.id` em entradas de contexto diferentes — ver §6.3 "Decisão F1"): o cupom que alcança a fração livre de uma linha parcialmente em conjunto é calculado sobre o subtotal já líquido do desconto do conjunto no mesmo item, não sobre o valor isolado da fração livre. F3/F4 evitam na prática usando linhas separadas (`metadata.conjunto_slot`) para peças elegíveis — **confirmado empiricamente na F3 (§6.3 "Confirmado F3")**: o metadata não funde linhas, então toda peça adicionada pela página do conjunto ou por "Complete o conjunto" fica isolada numa linha de quantidade 1. O limite continua valendo, sem contorno, para uma peça adicionada pelo botão comum "Adicionar à sacola" da PDP normal (sem `conjunto_slot`).
- **Par gerado por coleção pode ficar oculto da vitrine (F3, ruling V2)**: quando um curado ativo cobre exatamente o mesmo conjunto de produtos de um par de coleção, `listarConjuntos`/`conjuntoPorHandle` (`apps/backend/src/modules/beneficio-conjunto/catalogo-conjuntos.ts`) omitem o par — o carrinho aplicaria a regra do curado, não a da coleção, então mostrar as duas URLs seria inconsistente. Efeito colateral: `getElegibilidade()` da vitrine (que deriva as raízes elegíveis de `colecoes[].pares`) também deixava de marcar essa categoria com o selo "Forma conjunto", até o ruling V4 (§7) somar as raízes dos curados ativos à elegibilidade. Mudança de backend nesta fase (F3) — precisa de redeploy (`railway up`) antes do deploy da vitrine, senão a vitrine em produção roda contra o comportamento antigo do backend.
