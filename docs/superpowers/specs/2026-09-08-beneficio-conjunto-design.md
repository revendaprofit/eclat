...
Uma linha com N unidades das quais K estão marcadas (com desconto ou só "em conjunto") vira até 3 entradas de contexto com o **mesmo `id`** — cada campo monetário da entrada (não só `subtotal`: `total`, `discount_total`, `tax_total` etc., e o `raw_<campo>.value` correspondente) é reescalado por `n/q` para a fração de unidades daquela marca (§6.3 confirma que o motor aplica a promoção só à entrada marcada).
